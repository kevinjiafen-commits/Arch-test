    ...
    local _wallbangFunc = function()
        ...
        function obj:__desync_start(target)
            ...
            self.__conn2 = __y5z6a7.Heartbeat:Connect(function()
                ...
                -- knife-adaptive desync offset (MeleeDesyncScript port)
                local __viewMdls = __e1f2g3:FindFirstChild("ViewModels")
                local __hasKnife = false
                if __viewMdls then
                    for _, __mdl in __viewMdls:GetChildren() do
                        if __mdl:IsA("Model")
                           and string.find(__mdl.Name, target.Name, 1, true)
                           and string.find(__mdl.Name, "Knife", 1, true) then
                            __hasKnife = true; break
                        end
                    end
                end
                local __enemyHead = target.Character and target.Character:FindFirstChild("Head")
                local __dsp
                if __hasKnife then
                    __dsp = (enemyRoot.CFrame * CFrame.new(0, 6, 0)).Position
                else
                    __dsp = (enemyRoot.CFrame * CFrame.new(0, 1, 2)).Position
                end
                if __enemyHead then
                    myRoot.CFrame = CFrame.lookAt(__dsp, __enemyHead.Position)
                else
                    myRoot.CFrame = CFrame.new(__dsp)
                end
    ...
    local function _createWallbangHead()
        ...
        function obj:__desync_start(target)
            ...
            self.__conn2 = __y5z6a7.Heartbeat:Connect(function()
                ...
                -- knife-adaptive desync offset (MeleeDesyncScript port)
                local __enemyRoot = target.Character and target.Character:FindFirstChild("HumanoidRootPart")
                local __viewMdls2 = __e1f2g3:FindFirstChild("ViewModels")
                local __hasKnife2 = false
                if __viewMdls2 then
                    for _, __mdl in __viewMdls2:GetChildren() do
                        if __mdl:IsA("Model")
                           and string.find(__mdl.Name, target.Name, 1, true)
                           and string.find(__mdl.Name, "Knife", 1, true) then
                            __hasKnife2 = true; break
                        end
                    end
                end
                if __enemyRoot then
                    local __dsp2
                    if __hasKnife2 then
                        __dsp2 = (__enemyRoot.CFrame * CFrame.new(0, 6, 0)).Position
                    else
                        __dsp2 = (__enemyRoot.CFrame * CFrame.new(0, 1, 2)).Position
                    end
                    myRoot.CFrame = CFrame.lookAt(__dsp2, enemyHead.Position)
                else
...
-- knife ViewModel detector (MeleeDesyncScript port)
local function HasKnifeViewModel(targetPlayer)
    ...
    for _, mdl in viewModels:GetChildren() do
        if mdl:IsA("Model")
           and string.find(mdl.Name, tName, 1, true)
           and string.find(mdl.Name, "Knife", 1, true) then
            return true
        end
    end
    return false
...
-- SpectateController for duel-aware team resolution
local _cachedSpectateController = nil
local function ResolveSpectateController()
    if _cachedSpectateController then return _cachedSpectateController end
    local scripts = Player:FindFirstChild("PlayerScripts")
    local ctrl = scripts and scripts:FindFirstChild("Controllers")
    local mod = ctrl and ctrl:FindFirstChild("SpectateController")
    if mod then
        local ok, sc = pcall(require, mod)
        if ok and sc then _cachedSpectateController = sc end
    end
    return _cachedSpectateController
end
...
local function IsEnemyPlayer(player)
    ...
    -- duel-aware team check via SpectateController (MeleeDesyncScript port)
    local sc = ResolveSpectateController()
    if sc then
        local duel = sc.CurrentDuelSubject
        if duel then
            local localDueler = type(duel.GetDueler) == "function" and duel:GetDueler(Player) or nil
            local localTeamSC = localDueler and type(localDueler.Get) == "function" and localDueler:Get("TeamID") or nil
            if localTeamSC and duel.Duelers then
                for _, dueler in duel.Duelers do
                    if dueler.Player == player then
                        local t = type(dueler.Get) == "function" and dueler:Get("TeamID") or nil
                        if t then return t ~= localTeamSC end
                    end
                end
            end
        end
        ...
        if player and player ~= Player and IsEnemyPlayer(player) then
            ...
            if model then
                ...
                if head and root then
                    ...
                    -- katana deflect detection (MeleeDesyncScript port)
                    local __equipped = fighter.EquippedItem
                    local __isKatanaDeflect = false
                    if __equipped and __equipped.ViewModel and __equipped.ViewModel.Name == "Katana" then
                        __isKatanaDeflect = (__equipped._attack_cooldown and __equipped._attack_cooldown > tick()) or false
                    end
                    table.insert(out, {
                        fighter = fighter,
                        player = player,
...
local function IsValidTarget(entry)
    return entry and entry.alive and not entry.invincible and not entry.deflecting
end
...
function Controller:Update(dt)
    ...
    if target and action then
        ...
        if isMeleeAction and self._meleeStrategy then
            ...
            local knifeBoost = HasKnifeViewModel(target.player) and 4 or 0
            local hoverCF = ResolveRagebotCFrame(targetRoot, radius, height + knifeBoost, phase)
