...
function PartGlue:Acquire(ourPart, hitboxPart)
    ...
    -- Do NOT move hitboxPart — the server validates the hit part is still
    -- a valid descendant of the target's model. Moving it to void breaks that check.
    -- Return the hitbox's actual world CFrame so melee callers can offset from it.
...
function Controller:Update(dt)
    ...
    if target and action then
        ...
        if isMeleeAction and self._meleeStrategy then
            ...
            -- Acquire links PhysicsRepRootPart → hitboxHead (no longer moves the hitbox).
            -- SetServerCFrame to the hitboxHead's actual world position so the server's
            -- range/proximity check sees us co-located with the target hitbox.
            local hitboxCF = self._partGlue:Acquire(root, target.hitboxHead)
            charCtrl:SetServerCFrame(hitboxCF)
...
local function UpdateFlickbot(dt)
    ...
    if held and Flickbot.State == "Idle" then
        ...
        if camera and target then
            -- Use hitboxHead position for screen projection — rootPart may be at the
            -- server-desync position after ragebot runs, which WorldToViewportPoint
            -- would project off-screen even if the enemy is visible in front of us.
            local aimPart = target.hitboxHead or target.rootPart
            local aimPos = aimPart.Position
            local pt, onScreen = camera:WorldToViewportPoint(aimPos)
            -- 50px margin: accept targets barely outside the viewport edge
            if not onScreen then
                local vp = camera.ViewportSize
                onScreen = pt.X > -50 and pt.X < vp.X + 50
                         and pt.Y > -50 and pt.Y < vp.Y + 50
                         and pt.Z > 0
            end
            if onScreen then
                local vp = camera.ViewportSize
                ...
                if #traj > 1 then
                    ...
                    Flickbot.BakedEndDirection = (aimPos - camera.CFrame.Position).Unit
        ...
        if not nxt then
            if Settings.FlickbotShot then
                local function doFlickShot()
                    ...
                    local sel = Flickbot.Selected
                    local objId = itemObjectId(item)
                    if objId and sel then
                        -- Fire via server remote (same path as ragebot) so the hit
                        -- is registered server-side with the correct hitbox reference.
                        -- client-side StartShooting gets angle-validated and rejected.
                        local isRaycast = item.Info and item.Info.IsRaycast == true
                        local above = true
                        local aim1 = buildAim(above and AIM_ABOVE_ORIGIN or AIM_BELOW_ORIGIN, above and PITCH_ABOVE or PITCH_BELOW, 0, 0)
                        local aim2 = buildAim(above and AIM_ABOVE_END or AIM_BELOW_END, above and PITCH_ABOVE or PITCH_BELOW, 0, 0)
                        fireGun(objId, isRaycast, aim1, aim2, sel.hitboxHead, AIM_EXTRA)
                    elseif item.StartShooting then
                        pcall(item.StartShooting, item)
                    end
                end
                local delay = Settings.FlickbotShotDelay / 1000
                if delay > 0 then
                    task.delay(delay, doFlickShot)
                else
                    doFlickShot()
                end
            end
