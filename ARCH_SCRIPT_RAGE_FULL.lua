...
-- ============================================================================
-- Replication Rage (independent module)
-- Added separately from the existing Ragebot/Flickbot.  This module keeps its
-- own state, heartbeat, weapon selector and enable switch.
-- ============================================================================
local ReplicationRage = (function()
    local M = {}
    local Settings = {
        Enabled = false,
        FireRate = 0.0005,
        ...
        AutoEquip = true,
        MaxDistance = 500,
        IgnoreKatanaDeflect = true,
        KnifeOffsetY = 6,
        KnifeOffsetZ = 0,
        NormalOffsetY = 1,
        NormalOffsetZ = 2,
        RandomAim = true,
        RandomAimAmount = 0.10,
    }

    local Player = LocalPlayer
    local PlayersSvc = Players
    local ReplicatedStorageSvc = ReplicatedStorage
    local RunServiceSvc = RunService
    local WorkspaceSvc = Workspace
    local UserInputServiceSvc = UserInputService

    local FighterCtrl = nil
    local SpectateCtrl = nil
    local Utility = nil
    local EnumLib = nil
    ...
    local function resolveModules()
        if not FighterCtrl then
            ...
            FighterCtrl = safeRequire(module)
        end
        if not SpectateCtrl then
            local scripts = Player and Player:FindFirstChild("PlayerScripts")
            local controllers = scripts and scripts:FindFirstChild("Controllers")
            local module = controllers and controllers:FindFirstChild("SpectateController")
            SpectateCtrl = safeRequire(module)
        end
        local modules = ReplicatedStorageSvc and ReplicatedStorageSvc:FindFirstChild("Modules")
        if not Utility and modules then
            Utility = safeRequire(modules:FindFirstChild("Utility"))
        end
        if not EnumLib and modules then
            EnumLib = safeRequire(modules:FindFirstChild("EnumLibrary"))
        end
        return FighterCtrl ~= nil
    end

    local function getLocalFighter()
        resolveModules()
        return FighterCtrl and FighterCtrl.LocalFighter or nil
    end

    local function getSlotNumber()
        return slotNumbers[Settings.WeaponSlot] or 3
    end
    ...
    local function equipSelected()
        ...
        if fighter and type(fighter.EquipItem) == "function" then
            pcall(fighter.EquipItem, fighter, getSlotNumber())
        end
    end

    local function updateDeflection()
        resolveModules()
        if not FighterCtrl or type(FighterCtrl.Objects) ~= "table" then return end
        for _, fighterObj in pairs(FighterCtrl.Objects) do
            local player = fighterObj and fighterObj.Player
            if player then
                local entity = fighterObj.Entity
                local alive = entity and type(entity.IsAlive) == "function" and entity:IsAlive()
                local spectating = false
                pcall(function() spectating = fighterObj:Get("IsSpectating") == true end)
                if not alive or spectating then
                    deflecting[player] = false
                else
                    local equipped = fighterObj.EquippedItem
                    local isKatana = equipped
                        and equipped.ViewModel
                        and equipped.ViewModel.Name == "Katana"
                    local isDeflecting = false
                    if Settings.IgnoreKatanaDeflect and isKatana then
                        local cooldown = equipped._attack_cooldown
                        isDeflecting = (type(cooldown) == "number" and cooldown > tick()) or false
                    end
    ...
    local function isEnemy(player)
        ...
        resolveModules()
        local duel = SpectateCtrl and SpectateCtrl.CurrentDuelSubject
        local localDueler = nil
        if duel and type(duel.GetDueler) == "function" then
            pcall(function() localDueler = duel:GetDueler(Player) end)
        end
        local localTeam = nil
        if localDueler and type(localDueler.Get) == "function" then
            pcall(function() localTeam = localDueler:Get("TeamID") end)
        end
        if localTeam and duel and type(duel.Duelers) == "table" then
            for _, dueler in pairs(duel.Duelers) do
                if dueler and dueler.Player == player then
                    local team = nil
                    pcall(function() team = dueler:Get("TeamID") end)
                    return team ~= localTeam
                end
            end
        end
        local pTeam = player:GetAttribute("TeamID")
        local lTeam = Player:GetAttribute("TeamID")
        if pTeam ~= nil and lTeam ~= nil then
            return pTeam ~= lTeam
        end
        return true
    end
    ...
    local function getClosestTarget()
        ...
        local closestDist = tonumber(Settings.MaxDistance) or 500
    ...
    local function hasKnifeViewModel(targetPlayer)
        ...
        local viewModels = WorkspaceSvc:FindFirstChild("ViewModels")
    ...
    local function makeCameraData(originPos, targetHead)
        if not Utility or type(Utility.EncodeCFrame) ~= "function" then return nil end
        ...
        if Settings.RandomAim then
            local amount = tonumber(Settings.RandomAimAmount) or 0.10
        ...
        cameraData[utf8.char(1)] = {
            [utf8.char(0)] = Utility:EncodeCFrame(aimCF),
            [utf8.char(1)] = Utility:EncodeCFrame(targetCF),
            [utf8.char(2)] = targetHead,
            [utf8.char(3)] = Utility:EncodeCFrame(objSpaceHeadOffset),
        }
        return cameraData
    end

    local function temporaryDesync(targetPlayer, targetRoot, targetHead)
        local char = Player and Player.Character
        local myRoot = char and char:FindFirstChild("HumanoidRootPart")
        if not myRoot or not targetRoot or not targetHead then return nil end

        local desyncPos
        if hasKnifeViewModel(targetPlayer) then
            desyncPos = (targetRoot.CFrame * CFrame.new(0, Settings.KnifeOffsetY, Settings.KnifeOffsetZ)).Position
        else
            desyncPos = (targetRoot.CFrame * CFrame.new(0, Settings.NormalOffsetY, Settings.NormalOffsetZ)).Position
        end
        ...
        if restoreBound then
            pcall(function() RunServiceSvc:UnbindFromRenderStep("__ARCH_ReplicationRage_Restore") end)
            restoreBound = false
        end
        restoreBound = true
        RunServiceSvc:BindToRenderStep("__ARCH_ReplicationRage_Restore", 101, function()
    ...
    local function fire(targetPlayer, targetRoot, targetHead)
        if not Settings.Enabled then return end
        if deflecting[targetPlayer] then return end
        if tick() - lastFire < Settings.FireRate then return end
        local fighter = getLocalFighter()
        if not fighter then return end
        local item = fighter.EquippedItem
        if not item then return end
        local remotes = ReplicatedStorageSvc:FindFirstChild("Remotes")
        ...
        if not useItem or not Utility or not EnumLib then return end
        local objectID = nil
        pcall(function() objectID = item:Get("ObjectID") end)
        if objectID == nil then return end
        local enumValue = nil
        pcall(function() enumValue = EnumLib:ToEnum("StartShooting") end)
        if enumValue == nil then return end

        local desyncCF = temporaryDesync(targetPlayer, targetRoot, targetHead)
        local originPos = desyncCF and desyncCF.Position or targetRoot.Position
        local cameraData = makeCameraData(originPos, targetHead)
        if not cameraData then return end

        lastFire = tick()
        pcall(function()
            useItem:FireServer(objectID, enumValue, cameraData, nil)
        end)
    end
    ...
    local function step()
        if not Settings.Enabled then return end
        resolveModules()
        if not FighterCtrl then return end
        if Settings.AutoEquip then equipSelected() end
        updateDeflection()
        local targetPlayer, targetRoot, targetHead = getClosestTarget()
        if not targetPlayer or not targetRoot or not targetHead then return end
        fire(targetPlayer, targetRoot, targetHead)
    end
    ...
    function M:SetEnabled(value)
        ...
        if Settings.Enabled then
            ...
            if not heartbeatConn then
                heartbeatConn = RunServiceSvc.Heartbeat:Connect(function()
                    pcall(step)
                end)
            end
...
local ReplicationRageGroup = RagePlus:Group("Replication Rage")
ReplicationRageGroup:Toggle({Name="Enabled", Default=false, Callback=function(v)
    ReplicationRage:SetEnabled(v)
end})
ReplicationRageGroup:Dropdown({Name="Weapon Slot", Options={"Melee", "Primary", "Secondary"}, Default="Melee", Callback=function(v)
    ReplicationRage:SetSetting("WeaponSlot", v)
    if ReplicationRage:GetSetting("Enabled") then
        task.defer(function() pcall(function() ReplicationRage:EquipSelected() end) end)
    end
end})
ReplicationRageGroup:Toggle({Name="Auto Equip", Default=true, Callback=function(v)
    ReplicationRage:SetSetting("AutoEquip", v)
end})
ReplicationRageGroup:Slider({Name="Fire Rate", Min=0, Max=50, Default=0.5, Unit="ms", Callback=function(v)
    ReplicationRage:SetSetting("FireRate", v / 1000)
end})
ReplicationRageGroup:Slider({Name="Max Distance", Min=50, Max=1000, Default=500, Unit="studs", Callback=function(v)
    ReplicationRage:SetSetting("MaxDistance", v)
end})
ReplicationRageGroup:Toggle({Name="Ignore Katana Deflect", Default=true, Callback=function(v)
    ReplicationRage:SetSetting("IgnoreKatanaDeflect", v)
end})
ReplicationRageGroup:Toggle({Name="Random Aim", Default=true, Callback=function(v)
    ReplicationRage:SetSetting("RandomAim", v)
end})
ReplicationRageGroup:Slider({Name="Random Aim Amount", Min=0, Max=100, Default=10, Unit="%", Callback=function(v)
    ReplicationRage:SetSetting("RandomAimAmount", v / 100)
end})
ReplicationRageGroup:Slider({Name="Normal Offset Y", Min=-10, Max=10, Default=1, Callback=function(v)
    ReplicationRage:SetSetting("NormalOffsetY", v)
end})
ReplicationRageGroup:Slider({Name="Normal Offset Z", Min=-10, Max=10, Default=2, Callback=function(v)
    ReplicationRage:SetSetting("NormalOffsetZ", v)
end})
ReplicationRageGroup:Slider({Name="Knife Offset Y", Min=-10, Max=15, Default=6, Callback=function(v)
    ReplicationRage:SetSetting("KnifeOffsetY", v)
end})
ReplicationRageGroup:Slider({Name="Knife Offset Z", Min=-10, Max=10, Default=0, Callback=function(v)
    ReplicationRage:SetSetting("KnifeOffsetZ", v)
end})
