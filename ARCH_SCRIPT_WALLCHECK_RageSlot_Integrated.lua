...
-- [[ ARCHIVE RAGE SLOT MODULE ]]
-- Integrated from the supplied Rage script.
-- This module is intentionally isolated from the existing Rage/Rage+ systems.
do
    local RageSlotReplicatedStorage = ReplicatedStorage
    local RageSlotPlayers = Players
    local RageSlotRunService = _run
    local RageSlotWorkspace = _ws
    local RageSlotLocalPlayer = LocalPlayer
    local RageSlotUtility = require(RageSlotReplicatedStorage.Modules:WaitForChild("Utility"))
    local RageSlotEnum = require(RageSlotReplicatedStorage.Modules:WaitForChild("EnumLibrary"))
    local RageSlotSpectateController
    pcall(function()
        RageSlotSpectateController = require(Controllers:WaitForChild("SpectateController", 10))
    end)

    local RageSlotConfig = {
        Enabled = false,
        FireRate = 0.5,
        WeaponSlot = "Melee",
        MaxDistance = 500,
        ...
        NormalForward = 2,
        AutoEquip = true,
        SkipKatanaDeflect = true
    }

    local RageSlotNumbers = {
        Primary = 1,
        Secondary = 2,
        Melee = 3
    }

    local RageSlotLastFire = 0
    local RageSlotDeflecting = {}
    local RageSlotHeartbeat
    local RageSlotEquipThread

    local function RageSlotGetSlotNumber()
        return RageSlotNumbers[RageSlotConfig.WeaponSlot] or 3
    end

    local function RageSlotGetFighter()
        local fighter = nil
        pcall(function()
    ...
    local function RageSlotEquipSelected()
        if not RageSlotConfig.AutoEquip then return end
        local fighter = RageSlotGetFighter()
        if not fighter then return end
        pcall(function()
            fighter:EquipItem(RageSlotGetSlotNumber())
        end)
    end

    local function RageSlotStartEquipLoop()
        if RageSlotEquipThread then return end
        RageSlotEquipThread = task.spawn(function()
            while RageSlotConfig.Enabled do
                RageSlotEquipSelected()
                task.wait(1)
            end
            RageSlotEquipThread = nil
        end)
    end

    local function RageSlotIsEnemy(player)
        if not player or player == RageSlotLocalPlayer then return false end

        if RageSlotSpectateController then
            local ok, result = pcall(function()
                local duel = RageSlotSpectateController.CurrentDuelSubject
                local localDueler = duel and duel:GetDueler(RageSlotLocalPlayer)
                ...
                if localTeam and duel and duel.Duelers then
                    for _, dueler in duel.Duelers do
                        if dueler.Player == player then
                            return dueler:Get("TeamID") ~= localTeam
                        end
                    end
                end
                return nil
        ...
        local lTeam = RageSlotLocalPlayer:GetAttribute("TeamID")
    ...
    local function RageSlotGetClosestTarget()
        local char = RageSlotLocalPlayer.Character
        if not char then return nil, nil, nil end
        local myRoot = char:FindFirstChild("HumanoidRootPart")
        if not myRoot then return nil, nil, nil end

        local closestPlayer, closestRoot, closestHead
        local closestDist = RageSlotConfig.MaxDistance

        for _, player in RageSlotPlayers:GetPlayers() do
            if RageSlotIsEnemy(player) then
    ...
    local function RageSlotHasKnifeViewModel(targetPlayer)
        if not targetPlayer then return false end
        local viewModels = RageSlotWorkspace:FindFirstChild("ViewModels")
    ...
    local function RageSlotUpdateDeflection()
        ...
        for _, fighterObj in FighterController.Objects do
            ...
            if player then
                if not fighterObj.Entity or not fighterObj.Entity:IsAlive() or fighterObj:Get("IsSpectating") then
                    RageSlotDeflecting[player] = false
                else
                    ...
                    RageSlotDeflecting[player] = isDeflecting
                end
            end
        end
    end

    local function RageSlotBuildCameraData(aimCF, targetCF, targetHead, aimedPos)
        local objSpaceHeadOffset = targetHead.CFrame:ToObjectSpace(CFrame.new(aimedPos))
        local cameraData = {}
        cameraData[utf8.char(1)] = {
            [utf8.char(0)] = RageSlotUtility:EncodeCFrame(aimCF),
            [utf8.char(1)] = RageSlotUtility:EncodeCFrame(targetCF),
            [utf8.char(2)] = targetHead,
            [utf8.char(3)] = RageSlotUtility:EncodeCFrame(objSpaceHeadOffset)
        }
        return cameraData
    end

    local function RageSlotFire(targetPlayer, targetRoot, targetHead, desyncCF)
        if not RageSlotConfig.Enabled then return end
        if not targetPlayer or not targetRoot or not targetHead then return end
        if RageSlotConfig.SkipKatanaDeflect and RageSlotDeflecting[targetPlayer] then return end
        ...
        local now = tick()
        if now - RageSlotLastFire < RageSlotConfig.FireRate then return end
        RageSlotLastFire = now
        ...
        local spread = RageSlotConfig.RandomOffset
        ...
        local cameraData = RageSlotBuildCameraData(aimCF, targetCF, targetHead, aimedPos)

        local remotes = RageSlotReplicatedStorage:FindFirstChild("Remotes")
        ...
        pcall(function()
            useItem:FireServer(
                objectID,
                RageSlotEnum:ToEnum("StartShooting"),
                cameraData,
                nil
            )
        end)
    ...
    local function RageSlotStep()
        RageSlotUpdateDeflection()
        if not RageSlotConfig.Enabled then return end

        local targetPlayer, targetRoot, targetHead = RageSlotGetClosestTarget()
        if not targetPlayer or not targetRoot or not targetHead then return end

        local desyncCF
        local desyncPos
        if RageSlotHasKnifeViewModel(targetPlayer) then
            desyncPos = (targetRoot.CFrame * CFrame.new(0, RageSlotConfig.KnifeHeight, 0)).Position
        else
            desyncPos = (targetRoot.CFrame * CFrame.new(0, RageSlotConfig.NormalHeight, RageSlotConfig.NormalForward)).Position
        end
        ...
        if myRoot then
            ...
            RageSlotRunService:BindToRenderStep("__ARCH_RageSlotRestore", 101, function()
                ...
                pcall(function()
                    RageSlotRunService:UnbindFromRenderStep("__ARCH_RageSlotRestore")
                end)
            end)
        end

        RageSlotFire(targetPlayer, targetRoot, targetHead, desyncCF)
    end

    local function RageSlotSetEnabled(enabled)
        RageSlotConfig.Enabled = enabled and true or false
        if RageSlotConfig.Enabled then
            RageSlotEquipSelected()
            RageSlotStartEquipLoop()
            if not RageSlotHeartbeat then
                RageSlotHeartbeat = RageSlotRunService.Heartbeat:Connect(function()
                    pcall(RageSlotStep)
                end)
            end
        else
            if RageSlotHeartbeat then
                RageSlotHeartbeat:Disconnect()
                RageSlotHeartbeat = nil
            end
        end
    end

    RageSlotPlayers.PlayerRemoving:Connect(function(player)
        RageSlotDeflecting[player] = nil
    end)

    _G.ARCH_RageSlot = {
        Config = RageSlotConfig,
        SetEnabled = RageSlotSetEnabled,
        EquipSelected = RageSlotEquipSelected
    }
end
-- [[ END ARCHIVE RAGE SLOT MODULE ]]
...
local RageSlotGroup = Rage:Group("Rage Slot")
RageSlotGroup:Toggle({
    Name = "Enabled",
    Default = false,
    Tooltip = "Standalone Rage with selectable weapon slot",
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.SetEnabled then
            _G.ARCH_RageSlot.SetEnabled(v)
        end
    end
})
RageSlotGroup:Dropdown({
    Name = "Weapon Slot",
    Options = {"Primary", "Secondary", "Melee"},
    Default = "Melee",
    Tooltip = "Choose which weapon slot Rage equips and uses",
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.WeaponSlot = v
            if _G.ARCH_RageSlot.Config.Enabled then
                _G.ARCH_RageSlot.EquipSelected()
            end
        end
    end
})
RageSlotGroup:Slider({
    Name = "Fire Rate",
    Min = 0.01,
    Max = 1,
    Default = 0.5,
    Decimals = 2,
    Unit = "s",
    Tooltip = "Minimum time between Rage shots",
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.FireRate = v
        end
    end
})
RageSlotGroup:Slider({
    ...
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.MaxDistance = v
        end
    end
})
RageSlotGroup:Slider({
    ...
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.RandomOffset = v
        end
    end
})
RageSlotGroup:Toggle({
    Name = "Auto Equip",
    Default = true,
    Tooltip = "Keep the selected slot equipped while Rage is enabled",
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.AutoEquip = v
            if v and _G.ARCH_RageSlot.Config.Enabled then
                _G.ARCH_RageSlot.EquipSelected()
            end
        end
    end
...
RageSlotGroup:Toggle({
    Name = "Skip Katana Deflect",
    Default = true,
    Tooltip = "Do not fire at targets currently deflecting with Katana",
    Callback = function(v)
        if _G.ARCH_RageSlot and _G.ARCH_RageSlot.Config then
            _G.ARCH_RageSlot.Config.SkipKatanaDeflect = v
        end
    end
})
