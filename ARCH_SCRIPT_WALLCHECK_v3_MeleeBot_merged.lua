...
-- ============================================================
-- MELEEBOT MODULE
-- Standalone melee/knife autofire with desync, merged from
-- the second script. Runs its own Heartbeat loop; does NOT
-- touch Gun.StartShooting so it never collides with
-- wallbang / wallbangHead desync systems.
-- ============================================================
local MeleeBot = (function()
    local _Services = setmetatable({}, {
        __index = function(_, s) return cloneref(game:GetService(s)) end
    })
    local _plrs  = _Services.Players
    local _runS  = _Services.RunService
    local _repS  = _Services.ReplicatedStorage
    local _ws    = _Services.Workspace
    local _lp    = _plrs.LocalPlayer

    local _util  = require(_repS.Modules.Utility)
    local _enum  = require(_repS.Modules.EnumLibrary)
    local _FC    = nil  -- FighterController, resolved lazily
    local _SC    = nil  -- SpectateController, resolved lazily

    local _slots = { Primary = 1, Secondary = 2, Melee = 3 }

    local _conn       = nil   -- Heartbeat connection
    local _equipConn  = nil   -- keep-equip loop
    local _lastFire   = 0
    local _deflecting = {}

    -- ---- helpers ----------------------------------------

    local function _getFC()
        if not _FC then
            pcall(function()
                _FC = require(_lp.PlayerScripts.Controllers.FighterController)
            end)
        end
        return _FC
    end

    local function _getSC()
        if not _SC then
            pcall(function()
                _SC = require(_lp.PlayerScripts.Controllers:WaitForChild("SpectateController", 5))
            end)
        end
        return _SC
    end
    ...
    local function _updateDeflection()
        ...
        for _, fighterObj in fc.Objects do
            ...
            local isDefl = false
            if isKatana then
                isDefl = (equipped._attack_cooldown
                          and equipped._attack_cooldown > tick()) or false
            end
            _deflecting[player] = isDefl
        end
    end

    local function _isEnemy(player)
        if player == _lp then return false end
        if S.meleeBotTeamCheck then
            local sc = _getSC()
            local duel = sc and sc.CurrentDuelSubject
            local localDueler = duel and duel:GetDueler(_lp)
            ...
            local pT = player:GetAttribute("TeamID")
            local lT = _lp:GetAttribute("TeamID")
    ...
    local function _hasKnifeViewModel(targetPlayer)
        ...
        for _, model in viewModels:GetChildren() do
            if model:IsA("Model")
               and string.find(model.Name, tName, 1, true)
    ...
    local function _getClosestTarget()
        ...
        for _, player in _plrs:GetPlayers() do
            ...
            -- optional wall check
            if S.meleeBotWallCheck then
                if not isClearLineOfSight(myRoot.Position, pHead.Position, pChar) then
                    continue
                end
            end
    ...
    function M.Start()
        ...
        _equipConn = task.spawn(function()
            while S.meleeBotEnabled do
                local fc = _getFC()
                if fc and fc.LocalFighter then
                    pcall(function()
                        fc.LocalFighter:EquipItem(
                            _slots[S.meleeBotSlot] or 3
                        )
                    end)
                end
                task.wait(1)
        ...
        task.spawn(function()
            ...
            while not lf do
                task.wait(0.1)
                fc = _getFC()
                lf = fc and fc.LocalFighter
            end
            pcall(function() lf:EquipItem(_slots[S.meleeBotSlot] or 3) end)
        end)

        _conn = _runS.Heartbeat:Connect(function()
            if not S.meleeBotEnabled then return end

            _updateDeflection()

            local targetPlayer, targetRoot, targetHead = _getClosestTarget()

            -- desync CFrame: snap our root toward enemy to register
            -- hits through position desync, same pattern as wallbang
            if targetRoot and targetHead then
                ...
                if myRoot then
                    ...
                    _runS:BindToRenderStep("__mb_restore", 102, function()
                        if myRoot then
                            myRoot.CFrame  = oldCF
                            myRoot.Velocity    = oldVel
                            myRoot.RotVelocity = oldRV
                        end
                        _runS:UnbindFromRenderStep("__mb_restore")
                    end)
                end
            end
            ...
            local now = tick()
            if now - _lastFire < S.meleeBotFireRate then return end
            _lastFire = now

            local myRoot    = myChar:FindFirstChild("HumanoidRootPart")
            local originPos = myRoot and myRoot.Position or targetRoot.Position
            ...
            pcall(function()
                _repS.Remotes.Replication.Fighter.UseItem:FireServer(
                    item:Get("ObjectID"),
    ...
    function M.Stop()
        S.meleeBotEnabled = false
        if _conn then _conn:Disconnect(); _conn = nil end
        if _equipConn then task.cancel(_equipConn); _equipConn = nil end
        _runS:UnbindFromRenderStep("__mb_restore")
    end

    function M.Restart()
        M.Stop()
        S.meleeBotEnabled = true
        M.Start()
    end
...
local function toggleMeleeBot(on)
    S.meleeBotEnabled = on
    if on then MeleeBot.Start() else MeleeBot.Stop() end
end
...
local S = {
    ...
    -- MeleeBot
    meleeBotEnabled = false,
    meleeBotFireRate = 0.0005,
    meleeBotSlot = "Melee",
    meleeBotWallCheck = false,
    meleeBotTeamCheck = true,
}
...
function ApplyAllSettings()
    ...
    if S.meleeBotEnabled then toggleMeleeBot(true) end
...
-- MELEEBOT UI TAB
-- ============================================================
local MeleeBotTab = Library:Tab("MeleeBot", 4703884015)

local MBMainGroup = MeleeBotTab:Group("Melee Autofire")
MBMainGroup:Toggle({
    Name = "Enabled",
    Default = false,
    Callback = function(v)
...
MBMainGroup:Slider({
    ...
    Default = 0.0005,
    Callback = function(v)
        S.meleeBotFireRate = v
    end
})

local MBTargetGroup = MeleeBotTab:Group("Targeting")
MBTargetGroup:Toggle({
    Name = "Team Check",
    Default = true,
    Callback = function(v)
        S.meleeBotTeamCheck = v
    end
})
MBTargetGroup:Toggle({
    Name = "Wall Check",
...
Library:Notify("Arch Scripts + MeleeBot + Mobile ESP + Advanced Anti-Aim/Underground + Anti Katana + Slide Boost + Config loaded", "success")
