...
local RageCore = (function()
    ...
    pcall(function()
        local mod = ReplicatedStorage:FindFirstChild("Modules") and ReplicatedStorage.Modules:FindFirstChild("Utility")
        if mod then
            local ok, util = pcall(require, mod)
            if ok and type(util) == "table" then RageUtility = util end
        end
    end)
...
local function ResolveFighterController()
    if FighterController and type(FighterController) == "table" then
        if rawget(FighterController, "Objects") ~= nil or rawget(FighterController, "LocalFighter") ~= nil then
    ...
    local mod = controllers and controllers:FindFirstChild("FighterController")
    if mod then
        local ok, ctrl = pcall(require, mod)
        if ok and type(ctrl) == "table" then
            FighterController = ctrl
            if rawget(ctrl, "Objects") ~= nil then
                return ctrl
            end
        end
    end

    -- The live FighterController can be a runtime table rather than the
    -- ModuleScript return value. Prefer that table when it exposes Objects.
    if type(getgc) == "function" then
        local ok, gc = pcall(getgc, true)
        if ok and type(gc) == "table" then
            for _, candidate in ipairs(gc) do
                if type(candidate) == "table"
                    and rawget(candidate, "Objects") ~= nil
                    and rawget(candidate, "LocalFighter") ~= nil then
                    FighterController = candidate
                    return candidate
                end
            end
        end
...
local function CollectEnemies()
    local out = {}
    local ctrl = ResolveFighterController()
    local objects = ctrl and ctrl.Objects
    if type(objects) == "table" then
      for _, fighter in pairs(objects) do
        if type(fighter) ~= "table" then continue end
    ...
    -- Fallback: use live Player.Character data, matching the working Direct Rage/Wallbang path.
    if #out == 0 then
        local myRoot = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= Player and IsEnemyPlayer(player) then
                local model = player.Character
                local root = model and model:FindFirstChild("HumanoidRootPart")
                local head = model and (model:FindFirstChild("HitboxHead") or model:FindFirstChild("Head"))
                local hum = model and model:FindFirstChildOfClass("Humanoid")
                if root and head and (not hum or hum.Health > 0) then
                    table.insert(out, {
                        fighter = nil, player = player, entity = nil, model = model,
                        hitboxHead = head, rootPart = root, humanoid = hum,
                        alive = hum == nil or hum.Health > 0, invincible = false,
                        distance = myRoot and (root.Position - myRoot.Position).Magnitude or math.huge
                    })
                end
            end
        end
...
local function fireGun(objectId, isRaycast, aim1, aim2, hitboxHead, extra)
    ...
    if not remote or not objectId or not hitboxHead then return end
    local enum = ResolveEnumLibrary()
    local util = RageUtility
    if not enum or not util or type(util.EncodeCFrame) ~= "function" then return end
    local token = enum:ToEnum("StartShooting")
    if not token then return end
    local targetCF = hitboxHead.CFrame
    local aimCF = CFrame.lookAt(hitboxHead.Position - hitboxHead.CFrame.LookVector * 5, hitboxHead.Position)
    local cameraData = {
        [utf8.char(1)] = {
            [utf8.char(0)] = util:EncodeCFrame(aimCF),
            [utf8.char(1)] = util:EncodeCFrame(targetCF),
            [utf8.char(2)] = hitboxHead,
            [utf8.char(3)] = util:EncodeCFrame(CFrame.new())
        }
    }
    pcall(function() remote:FireServer(objectId, token, cameraData, nil) end)
end

local function fireMeleeAttack(objectId, a, b, c, d)
    if not objectId or not c then return end
    local remote = ReplicatedStorage:FindFirstChild("Remotes")
        and ReplicatedStorage.Remotes:FindFirstChild("Replication")
        and ReplicatedStorage.Remotes.Replication.Fighter:FindFirstChild("UseItem")
    local enum = ResolveEnumLibrary()
    local util = RageUtility
    if not remote or not enum or not util or type(util.EncodeCFrame) ~= "function" then return end
    local token = enum:ToEnum("StartShooting")
    if not token then return end
    local aimCF = CFrame.lookAt(c.Position - c.CFrame.LookVector * 5, c.Position)
    local data = {[utf8.char(1)] = {[utf8.char(0)] = util:EncodeCFrame(aimCF), [utf8.char(1)] = util:EncodeCFrame(c.CFrame), [utf8.char(2)] = c, [utf8.char(3)] = util:EncodeCFrame(CFrame.new())}}
    pcall(function() remote:FireServer(objectId, token, data, nil) end)
end

local function fireMeleeHeavy(objectId, a, b, c, d)
    return fireMeleeAttack(objectId, a, b, c, d)
end
...
function Controller:Update(dt)
    ...
    -- Some live FighterController builds expose the equipped item but not a
    -- numeric-keyed Items table. Fall back to the same EquippedItem object
    -- used by the working Direct Rage module.
    if target and fighter and not action then
        local equipped = fighter.EquippedItem
        if equipped then
            action = { type = "Attack", item = equipped, index = nil }
        end
    end
...
local function UpdateFlickbot(dt)
    ...
    if held and Flickbot.State == "Idle" then
        ...
        if camera and target then
            local aimPart = target.hitboxHead or target.rootPart
            local pt, onScreen = camera:WorldToViewportPoint(aimPart.Position)
            if onScreen then
                local vp = camera.ViewportSize
                local mouse = UserInputService:GetMouseLocation()
                local startX = (mouse and mouse.X) or (vp.X / 2)
                local startY = (mouse and mouse.Y) or (vp.Y / 2)
                local traj = GenerateFlickbotTrajectory(startX, startY, pt.X, pt.Y)
                if #traj > 1 then
    ...
    if Flickbot.State == "Flicking" then
        ...
        if camera then
            ...
            local camCtrl = CameraControllerAimbot or GetCameraController()
            if camCtrl and type(camCtrl.MimicRotation) == "function" then
                pcall(function() camCtrl:MimicRotation(targetCF) end)
            elseif camCtrl and type(camCtrl.SetRotation) == "function" then
                local pitch, yaw = targetCF:ToOrientation()
                pcall(function() camCtrl:SetRotation(Vector2.new(pitch, yaw)) end)
            else
                camera.CFrame = targetCF
            end
        end
...
-- [[ Direct Rage Module ]] -- integrated from the supplied Rage implementation
local DirectRage = (function()
    local Settings = {
        Enabled = false,
        FireRate = 0.0005,
        WeaponSlot = "Melee",
    ...
    local equipLoopToken = 0

    local function getSlotNumber()
        return slots[Settings.WeaponSlot] or 3
    end
    ...
    local function getClosestTarget()
        ...
        for _, player in plrs:GetPlayers() do
            ...
            if dist < closestDist then
                closestDist = dist
                closestPlayer, closestRoot, closestHead = player, pRoot, pHead
            end
        end
    ...
    local function getDesyncCF(targetRoot, targetHead, targetPlayer)
        if not targetRoot or not targetHead then return nil end
        local desyncPos
        if hasKnifeViewModel(targetPlayer) then
            desyncPos = (targetRoot.CFrame * CFrame.new(0, Settings.KnifeDesyncHeight, 0)).Position
        else
            desyncPos = (targetRoot.CFrame * CFrame.new(0, Settings.NormalDesyncHeight, Settings.NormalDesyncZ)).Position
        end
        return CFrame.lookAt(desyncPos, targetHead.Position)
    end
    ...
    local function startEquipLoop()
        equipLoopToken += 1
        local token = equipLoopToken
        task.spawn(function()
            while Settings.Enabled and token == equipLoopToken do
                equipSelected()
                task.wait(Settings.ReequipInterval)
            end
        end)
    end
    ...
    local function fireAt(targetHead, targetRoot, desyncCF)
        resolveModules()
        if not FighterController or not util or not enum then return end
        local fighter = FighterController.LocalFighter
        ...
        local offsetScale = Settings.RandomAimOffset
        ...
        local useItem = fighterRemotes and fighterRemotes:FindFirstChild("UseItem")
    ...
    local function step()
        ...
        local desyncCF = getDesyncCF(targetRoot, targetHead, targetPlayer)
        ...
        if desyncCF and lplr.Character then
            ...
            if myRoot then
                ...
                runS:BindToRenderStep("__arch_direct_rage_restore", 101, function()
                    if myRoot and myRoot.Parent then
                        myRoot.CFrame = oldCF
                        myRoot.Velocity = oldVel
                        myRoot.RotVelocity = oldRotVel
                    end
                    pcall(function() runS:UnbindFromRenderStep("__arch_direct_rage_restore") end)
                end)
            end
        end

        if not targetPlayer or not targetHead or not targetRoot then return end
        if Settings.IgnoreKatanaDeflect and deflecting[targetPlayer] then return end
        if not lplr.Character or not lplr.Character:FindFirstChild("HumanoidRootPart") then return end
        fireAt(targetHead, targetRoot, desyncCF)
    end
    ...
    local function setWeaponSlot(value)
        if slots[value] then
            Settings.WeaponSlot = value
            if Settings.Enabled then equipSelected() end
        end
    end

    playerRemovingConnection = plrs.PlayerRemoving:Connect(function(player)
...
local DirectRageGroup = RagePlus:Group("Direct Rage / Weapon Slot")
DirectRageGroup:Toggle({Name="Enabled",Default=false,Tooltip="Direct target-fire rage module",Callback=function(v) DirectRage.SetEnabled(v) end})
DirectRageGroup:Dropdown({Name="Weapon Slot",Options={"Primary","Secondary","Melee"},Default="Melee",Tooltip="Select the slot used by Direct Rage",Callback=function(v) DirectRage.SetWeaponSlot(v) end})
DirectRageGroup:Slider({Name="Fire Rate",Min=0.0005,Max=0.1,Default=0.0005,Decimals=4,Unit="s",Callback=function(v) DirectRage.Settings.FireRate=v end})
DirectRageGroup:Slider({Name="Target Distance",Min=25,Max=500,Default=500,Unit=" studs",Callback=function(v) DirectRage.Settings.MaxDistance=v end})
DirectRageGroup:Slider({Name="Aim Randomness",Min=0,Max=0.5,Default=0.1,Decimals=2,Callback=function(v) DirectRage.Settings.RandomAimOffset=v end})
DirectRageGroup:Toggle({Name="Anti-Katana Deflect",Default=true,Tooltip="Do not fire while a detected Katana deflect is active",Callback=function(v) DirectRage.Settings.IgnoreKatanaDeflect=v end})
DirectRageGroup:Slider({Name="Re-equip Interval",Min=0.1,Max=3,Default=1,Decimals=1,Unit="s",Callback=function(v) DirectRage.Settings.ReequipInterval=v end})
