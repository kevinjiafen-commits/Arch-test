...
local function ResolveLocalFighter()
    -- FIX: use getgc-based resolver to get the live singleton, not the module table
    local ctrl = resolveFighterController()
    if not ctrl then
        -- fallback to module-level require path
        ctrl = ResolveFighterController()
    end
...
local function CollectEnemies()
    local out = {}
    -- FIX: use getgc-based resolver to get the live singleton with .Objects populated
    local ctrl = resolveFighterController()
    if not ctrl then ctrl = ResolveFighterController() end
...
local function itemCategory(idx)
    -- FIX: Rivals keys items by ObjectID (not slot index 1/2/3).
    -- Derive category from item Info instead of relying on numeric slot key.
    return "Primary" -- fallback; real category resolved in isCategoryEnabled via Info
end

local function isCategoryEnabled(cat, item)
    -- FIX: use item.Info.Type/Class to decide category, ignore slot key
    if item and item.Info then
        local t = item.Info.Type or item.Info.Class or ""
        if t == "Melee" then return Settings.WeaponMelee end
        if t == "Secondary" or t == "Pistol" then return Settings.WeaponSecondary end
        return Settings.WeaponPrimary -- Gun, Rifle, Sniper, etc.
    end
    -- no Info: allow through (better to fire than never fire)
    return Settings.WeaponPrimary
end
...
local function getAction(fighter)
    ...
    for slotKey, item in next, items do
        if type(item) == "table" then
            ...
            if cat and isCategoryEnabled(cat, item) then
                anyEnabled = true
                local prio = 1
...
function PartGlue:Acquire(ourPart, hitboxPart)
    -- FIX: attempt identity elevation for PhysicsRepRootPart glue;
    -- if sethiddenproperty does not take (executor without support), fall back to
    -- direct CFrame desync so the hitboxHead position is still used server-side.
    local prev = getthreadidentity()
    if setthreadidentity then pcall(setthreadidentity, 8) end
    local glueOk = pcall(sethiddenproperty, ourPart, "PhysicsRepRootPart", hitboxPart)
    if setthreadidentity then pcall(setthreadidentity, prev) end
    self._bindings[ourPart] = hitboxPart
    self._glueOk = self._glueOk or glueOk
    local cf = CFrame.new(math.random(-100000,-10000), 100000, math.random(-100000,10000))
    if glueOk then
        -- full glue path: move hitbox to void, server reads ourPart as if at that pos
        hitboxPart.CFrame = cf
        return CFrame.new(cf.Position)
    else
        -- fallback: place ourPart directly on the hitboxHead each heartbeat via CharacterController
        -- return hitboxHead's current world CFrame so SetServerCFrame positions us there
    ...
    if action and action.type == "Attack" then
        ...
        if objId and target then
            if item.Info and item.Info.Type == "Gun" then
                ...
                -- FIX: derive above/below from shield state; pass real target orientation
                local above = isAbove(target)
                local _px, _oy, _oz = target.rootPart.CFrame:ToOrientation()
                local aim1 = buildAim(above and AIM_ABOVE_ORIGIN or AIM_BELOW_ORIGIN, above and PITCH_ABOVE or PITCH_BELOW, _oy, _oz)
                local aim2 = buildAim(above and AIM_ABOVE_END or AIM_BELOW_END, above and PITCH_ABOVE or PITCH_BELOW, _oy, _oz)
...
local function UpdateFlickbot(dt)
    ...
    if Flickbot.State == "Flicking" then
        ...
        -- FIX: Rivals' CameraController does not expose SetRotation publicly.
        -- Use CameraControllerAimbot.MimicRotation (same module, already required above).
        -- MimicRotation accepts a full CFrame and handles the internal rotation state.
        local camera = Workspace.CurrentCamera
        if camera then
            local pos = camera.CFrame.Position
            local targetCF = CFrame.lookAt(pos, pos + dir)
            if CameraControllerAimbot and type(CameraControllerAimbot.MimicRotation) == "function" then
                pcall(function() CameraControllerAimbot:MimicRotation(targetCF) end)
            else
                -- last-resort: write camera CFrame directly; works in solo/private servers
                -- where the CameraController isn't overwriting every frame
                camera.CFrame = targetCF
            end
        end
