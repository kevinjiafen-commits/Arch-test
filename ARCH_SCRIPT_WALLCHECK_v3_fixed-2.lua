    ...
    if target and action then
        ...
        if isMeleeAction and self._meleeStrategy then
            ...
            if IsCandidatePositionInBounds(hoverCF.Position) then
                -- register hitbox glue for server-side hit detection
                self._partGlue:Acquire(root, target.hitboxHead)
                -- but send the orbit CFrame (near target) as server position,
                -- NOT the void glue CFrame — that's what caused the void bug
                charCtrl:SetServerCFrame(hoverCF)
...
local function UpdateFlickbot(dt)
    ...
    if held and Flickbot.State == "Idle" then
        ...
        if camera and target then
            -- Use hitboxHead for screen projection — it's the real client-visible part;
            -- rootPart may be at void coords if ragebot desync is active
            ...
            -- also accept targets just outside viewport (within 50px margin) to avoid
            -- the "never starts" failure when the part is barely clipped
        ...
        if not nxt then
            if Settings.FlickbotShot then
                local function doFlickShot()
                    ...
                    -- prefer fireGun (server remote) so hit registers server-side
                    local objId = itemObjectId(item)
