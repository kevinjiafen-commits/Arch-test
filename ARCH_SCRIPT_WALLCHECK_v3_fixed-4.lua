...
-- Flickbot: fallback target finder using Players list directly,
-- in case ResolveFighterController / getgc hasn't populated yet.
local function FlickbotSelectTarget()
    -- 优先走 RageCore 的 SelectTarget（依赖 FighterController.Objects）
    local entry = SelectTarget()
    if entry then return entry end
    -- fallback: 直接扫 Players，用 HitboxHead 或 Head
    local cam = Workspace.CurrentCamera
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if not char then continue end
        local root = char:FindFirstChild("HumanoidRootPart")
        local hitbox = char:FindFirstChild("HitboxHead") or char:FindFirstChild("Head")
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not (root and hitbox and hum and hum.Health > 0) then continue end
        local toTarget = (root.Position - camPos).Unit
        local angle = math.acos(math.clamp(camDir:Dot(toTarget), -1, 1))
        if angle < bestAngle then
            bestAngle = angle
            bestEntry = { rootPart = root, hitboxHead = hitbox, player = p, alive = true, invincible = false }
        end
    end
    return bestEntry
end
...
local function UpdateFlickbot(dt)
    ...
    -- 问题一修复: Unknown 键在 Roblox 里 IsKeyDown 恒 false，需要 guard
    ...
    -- 单次边沿触发: 只在 held 从 false→true 时启动新 flick，防止按住连续重触发
    if held and not Flickbot.WasHeld and Flickbot.State == "Idle" then
        local camera = Workspace.CurrentCamera
        local target = FlickbotSelectTarget()
    ...
    if Flickbot.State == "Flicking" then
        ...
        while idx < #traj and traj[idx+1] and traj[idx+1].t <= Flickbot.ElapsedMs do
            idx = idx + 1
        end
        ...
        -- 问题二修复: 优先走 MimicRotation，fallback 绑 RenderStep 在相机系统后写入
        ...
        if CameraControllerAimbot and CameraControllerAimbot.MimicRotation then
            ...
            RunService:BindToRenderStep("FlickbotCamWrite", Enum.RenderPriority.Camera.Value + 1, function()
                local cam = Workspace.CurrentCamera
                if cam then cam.CFrame = targetCF end
                RunService:UnbindFromRenderStep("FlickbotCamWrite")
            end)
        end
