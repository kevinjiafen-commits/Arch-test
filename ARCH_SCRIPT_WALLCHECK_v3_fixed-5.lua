...
-- ============================================================
-- FLICKBOT — 完全重写版
-- 不依赖 RageCore 内部 (getgc / FighterController.Objects)
-- 相机写入走独立 RenderStepped 连接，优先级高于游戏相机系统
-- 触发: 按下 keybind 瞬间 (边沿) 执行一次 flick
-- ============================================================

local _fb = {
    active      = false,   -- 当前是否正在 flick
    cooldown    = false,   -- 是否在冷却
    cooldownEnd = 0,
    wasHeld     = false,
    startCF     = nil,     -- flick 开始时的相机 CFrame
    targetPos   = nil,     -- 目标世界坐标
    startTime   = 0,
    conn        = nil,     -- RenderStepped 连接
}

-- 找最近的敌人，直接扫 Players，不走 getgc
local function _fbFindTarget()
    local cam = workspace.CurrentCamera
    if not cam then return nil end
    local camPos = cam.CFrame.Position
    local camDir = cam.CFrame.LookVector
    local bestDeg = math.huge
    local bestPos = nil
    for _, p in ipairs(Players:GetPlayers()) do
        if p == LocalPlayer then continue end
        if p:GetAttribute("TeamID") == LocalPlayer:GetAttribute("TeamID") then continue end
        local char = p.Character
        if not char then continue end
        -- 优先 HitboxHead（Rivals 自定义碰撞体），fallback Head
        ...
        local toTarget = (part.Position - camPos).Unit
        local deg = math.deg(math.acos(math.clamp(camDir:Dot(toTarget), -1, 1)))
        if deg < bestDeg then
            bestDeg = deg
            bestPos = part.Position
        end
    end
    return bestPos  -- Vector3 或 nil
end

-- 把相机平滑插值到 targetCF，写入走 RenderStepped 最高优先级
local function _fbStartFlick(targetPos)
    local cam = workspace.CurrentCamera
    if not cam then return end
    _fb.startCF   = cam.CFrame
    _fb.targetPos = targetPos
    _fb.startTime = tick()
    _fb.active    = true

    -- 断开旧连接
    if _fb.conn then _fb.conn:Disconnect(); _fb.conn = nil end

    _fb.conn = RunService.RenderStepped:Connect(function()
        if not _fb.active then
            if _fb.conn then _fb.conn:Disconnect(); _fb.conn = nil end
            return
        end

        local cfg = Settings  -- RageCore 闭包内的 Settings
        local duration = (cfg.FlickbotDuration or 110) / 1000  -- 秒
        local elapsed  = tick() - _fb.startTime
        local t        = math.clamp(elapsed / duration, 0, 1)

        -- 缓动: 前半段快速加速，后半段平稳落点 (smoothstep 变体)
        local eased = t < 0.5
            and (4 * t * t * t)
            or  (1 - (-2 * t + 2)^3 / 2)
        ...
        local startDir = _fb.startCF.LookVector
        local endDir   = (_fb.targetPos - camPos).Unit
        local dir      = startDir:Lerp(endDir, eased).Unit
        local newCF    = CFrame.lookAt(camPos, camPos + dir)

        -- 写入相机
        if CameraControllerAimbot and CameraControllerAimbot.MimicRotation then
            pcall(CameraControllerAimbot.MimicRotation, CameraControllerAimbot, newCF)
        else
            currentCam.CFrame = newCF
        end
        ...
        if t >= 1 then
            _fb.active      = false
            _fb.cooldown    = true
            _fb.cooldownEnd = tick() + (cfg.FlickbotCooldown or 250) / 1000

            -- 自动射击
            if cfg.FlickbotShot then
                local fireDelay = (cfg.FlickbotShotDelay or 0) / 1000
...
local function UpdateFlickbot(_dt)
    local cfg = Settings
    if not cfg.FlickbotEnabled then
        _fb.active   = false
        _fb.cooldown = false
        _fb.wasHeld  = false
        if _fb.conn then _fb.conn:Disconnect(); _fb.conn = nil end
    ...
    -- 冷却计时
    if _fb.cooldown then
        if tick() >= _fb.cooldownEnd then
            _fb.cooldown = false
        else
            return
        end
    ...
    -- 已在 flicking，让 RenderStepped 连接自己处理，这里不干预
    if _fb.active then return end

    -- keybind 检查 — Unknown 永远 false，加 guard
    local key      = cfg.FlickbotKeybind
    local validKey = key and key ~= Enum.KeyCode.Unknown
    local held     = validKey and UserInputService:IsKeyDown(key)

    -- 边沿触发: 只在按下瞬间执行一次
    if held and not _fb.wasHeld then
        local tPos = _fbFindTarget()
        if tPos then
            _fbStartFlick(tPos)
        end
    end
