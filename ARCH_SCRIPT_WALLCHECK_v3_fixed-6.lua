...
-- ============================================================
-- FLICKBOT — 持续追踪版（移动端适配）
-- 开关打开后每帧持续把准星平滑拉向最近敌人的 HitboxHead/Head
-- 不需要 keybind，不依赖 getgc / FighterController.Objects
-- ============================================================

local _fb = {
    conn = nil,  -- RenderStepped 连接句柄
}

-- 找最近敌人，返回目标 part（HitboxHead 优先，fallback Head）
...
-- 写相机方向
local function _fbWriteCamera(newCF)
    if CameraControllerAimbot and CameraControllerAimbot.MimicRotation then
        pcall(CameraControllerAimbot.MimicRotation, CameraControllerAimbot, newCF)
    else
        local cam = workspace.CurrentCamera
        if cam then cam.CFrame = newCF end
    end
end

local function _fbStop()
    if _fb.conn then
        _fb.conn:Disconnect()
        _fb.conn = nil
    end
...
local function _fbStart()
    if _fb.conn then return end  -- 已在运行
    _fb.conn = RunService.RenderStepped:Connect(function(dt)
        local cfg = Settings
        ...
        local target = _fbFindTarget()
        if not target then return end

        -- smooth 系数：FlickbotHumanness 越高越慢越人性化
        -- humanness 0 = 几乎瞬间锁定，100 = 最慢
        local humanness = math.clamp((cfg.FlickbotHumanness or 30) / 100, 0, 1)
        local speed     = 1 - humanness * 0.92   -- 范围 0.08 ~ 1.0
        local factor    = 1 - math.pow(1 - speed, dt * 60)  -- 帧率无关插值系数

        local camPos   = cam.CFrame.Position
        local camDir   = cam.CFrame.LookVector
        local targetDir = (target.Position - camPos).Unit
        local newDir   = camDir:Lerp(targetDir, math.clamp(factor, 0, 1)).Unit
        local newCF    = CFrame.lookAt(camPos, camPos + newDir)

        _fbWriteCamera(newCF)

        -- 自动射击：准星和目标夹角小于 2° 时开火
        if cfg.FlickbotShot then
            local angle = math.deg(math.acos(math.clamp(newDir:Dot(targetDir), -1, 1)))
            if angle < 2 then
                task.spawn(function()
                    task.wait((cfg.FlickbotShotDelay or 0) / 1000)
                    local fighter = ResolveLocalFighter()
...
FlickbotGroup:Toggle({Name="Enabled", Default=false, Tooltip="开启后持续把准星拉向最近敌人", Callback=function(v) if RageSettings then RageSettings.FlickbotEnabled=v end end})
FlickbotGroup:Toggle({Name="Auto Shoot", Default=false, Tooltip="准星对齐后自动开火", Callback=function(v) if RageSettings then RageSettings.FlickbotShot=v end end})
...
FlickbotGroup:Slider({Name="Smoothness", Min=0, Max=100, Default=30, Unit="%", Tooltip="0=瞬间锁定 100=最慢最顺滑", Callback=function(v) if RageSettings then RageSettings.FlickbotHumanness=v end end})
