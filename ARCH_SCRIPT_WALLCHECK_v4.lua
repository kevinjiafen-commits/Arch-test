...
local S = {
    ...
    aimbotTargetNPC = false, -- 人偶/标把: aimbot 也锁练习场靶子和人偶
    wallCheck = true,
    teamCheck = false,
    fireDelay = 1,
...
-- ── 人偶 / 标把 模型收集 (模块级，aimbot 和 silent aim 共用) ────────────────
local _NPC_CACHE     = {}
...
local function _collectNPCModels()
    ...
    -- 路线 B: 扫 Workspace，按 HitboxHead 识别（标把可能无 Humanoid）
    if #out == 0 then
...
local function toggleAimbot(on)
    ...
    if on then
        ...
        aimbotConn = RunService.RenderStepped:Connect(function(dt)
            ...
            -- ② 人偶 / 标把 (aimbot NPC 锁定)
            if S.aimbotTargetNPC then
                local partNames = {"HitboxHead", "HitboxBody", "Head", "HumanoidRootPart"}
                if S.aimTargetPart == "HumanoidRootPart" then
                ...
                for _, npcModel in ipairs(_collectNPCModels()) do
                    ...
                    for _, pname in ipairs(partNames) do
                        ...
                        if part and part:IsA("BasePart") then
                            ...
                            local degrees = math.deg(angle)
                            if degrees <= closestAngle then
                                if isClearLineOfSight(camPos, part.Position, npcModel) then
                                    closestAngle = degrees
                                    targetPart = part
                                end
                            end
...
local function applyGunEnhancements()
    ...
    if S.silentAimActive or S.noShootAnimEnabled then
        ...
        -- _collectNPCModels 已提升至模块级，aimbot 和 silent aim 共用同一缓存
...
AimGroup:Toggle({Name = "Target NPC (人偶/标把)", Default = false, Tooltip = "Aimbot 也锁定练习场人偶和射击靶子", Callback = function(v) S.aimbotTargetNPC = v end})
