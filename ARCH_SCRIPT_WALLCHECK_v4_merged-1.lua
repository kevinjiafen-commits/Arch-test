...
-- SpectateController: duel-aware team detection (ported from melee desync script)
local _spectateCtrl
pcall(function()
    _spectateCtrl = require(_pscripts.Controllers:WaitForChild("SpectateController", 10))
end)
...
local S = {
    ...
    -- Melee Direct Fire (desync remote-fire module)
    meleeDirectActive  = false,
    meleeDirectSlot    = "Melee",   -- "Primary" | "Secondary" | "Melee"
    meleeDirectRate    = 0.0005,    -- seconds between shots (0.0005 = ~2000/s)
}
...
local function isEnemy(player)
    ...
    -- SpectateController duel check: accurate during 1v1 duels
    if _spectateCtrl then
        local ok, result = pcall(function()
            local duel = _spectateCtrl.CurrentDuelSubject
    ...
    -- Fallback: attribute-based team check
...
-- Knife viewmodel check: if target is holding a Knife, offset desync position higher
-- (avoids the blade catching the hitbox from below)
...
-- ── Melee Direct Fire Module ───────────────────────────────────────────────
-- Ports the desync+remote-fire loop from the Rivals melee spam script.
-- Each Heartbeat tick:
--   1. Auto-equips the configured weapon slot.
--   2. Finds closest living enemy (duel-aware via SpectateController).
--   3. Computes a desync CFrame offset beside the enemy head
--      (higher if they hold a Knife, to avoid the blade).
--   4. Briefly moves localHRP to that CFrame, fires UseItem via
--      arch's existing fireServerNative, then restores HRP on RenderStep.
-- Katana deflect is respected via the existing AntiKatana module.
-- ─────────────────────────────────────────────────────────────────────────

local MeleeDirectModule = (function()
    local _active   = false
    local _lastFire = 0
    local _deflecting = {}

    local _slotMap = { Primary = 1, Secondary = 2, Melee = 3 }

    -- cached Utility module for EncodeCFrame
    local _util = nil
    local function _getUtil()
        if _util then return _util end
        local ok, u = pcall(function()
            return require(ReplicatedStorage:FindFirstChild("Modules"):FindFirstChild("Utility"))
        end)
        if ok and u then _util = u end
        return _util
    end

    -- cached UseItem remote
    local _useItemRemote = nil
    local function _getRemote()
        if _useItemRemote and _useItemRemote.Parent then return _useItemRemote end
        local r = ReplicatedStorage:FindFirstChild("Remotes")
        r = r and r:FindFirstChild("Replication")
        r = r and r:FindFirstChild("Fighter")
        r = r and r:FindFirstChild("UseItem")
        if r then _useItemRemote = r end
        return _useItemRemote
    end

    local function _updateDeflection()
        local fc = FighterController
        if not fc or not fc.Objects then return end
        for _, fo in fc.Objects do
            local p = fo.Player
            if not p then continue end
            local alive = fo.Entity and (
                type(fo.Entity.IsAlive) == "function" and fo.Entity:IsAlive()
                or fo.Entity.Health == nil or fo.Entity.Health > 0
            )
            if not alive or (fo.Get and fo:Get("IsSpectating")) then
                _deflecting[p] = false
                continue
            end
            local eq = fo.EquippedItem
            local isKatana = eq and eq.ViewModel and eq.ViewModel.Name == "Katana"
            if isKatana and eq._attack_cooldown then
                _deflecting[p] = eq._attack_cooldown > tick()
            else
                _deflecting[p] = false
            end
        end
    ...
    -- Closest enemy by HRP distance, duel-aware via upgraded isEnemy()
    local function _getTarget()
        local char = LocalPlayer.Character
        if not char then return nil, nil, nil end
        local myRoot = char:FindFirstChild("HumanoidRootPart")
        if not myRoot then return nil, nil, nil end
        local bestPlayer, bestRoot, bestHead = nil, nil, nil
        local bestDist = 500
        for _, p in Players:GetPlayers() do
            ...
            if not (pr and ph and hum and hum.Health > 0) then continue end
    ...
    -- Auto-equip the configured slot every ~60 frames when active
    local _equipFrame = 0
    local function _autoEquip()
        _equipFrame = _equipFrame + 1
        if _equipFrame % 60 ~= 0 then return end
        local fc = FighterController
        local lf = fc and fc.LocalFighter
        if not lf then return end
        local slot = _slotMap[S.meleeDirectSlot] or 3
        pcall(function() lf:EquipItem(slot) end)
    end

    local _mainConn
    local function _start()
        if _mainConn then _mainConn:Disconnect() end
        _mainConn = RunService.Heartbeat:Connect(function()
            if not _active then return end
            _autoEquip()
            _updateDeflection()

            local targetPlayer, targetRoot, targetHead = _getTarget()
            local desyncCF = nil

            -- Build desync offset: knife users get a higher offset to avoid
            -- the blade geometry clipping through the shot origin.
            ...
            -- Server-side position desync: move HRP to fire origin, restore
            -- at RenderStep priority 101 (before camera, after physics).
            if desyncCF and LocalPlayer.Character then
                ...
                if myRoot then
                    ...
                    RunService:BindToRenderStep("__mdd_restore", 101, function()
                        if myRoot then
                            myRoot.CFrame     = oldCF
                            myRoot.Velocity   = oldVel
                            myRoot.RotVelocity = oldRV
                        end
                        RunService:UnbindFromRenderStep("__mdd_restore")
                    end)
                end
            end

            if not targetPlayer or not targetHead or not targetRoot then return end
            if _deflecting[targetPlayer] then return end
            ...
            local now = tick()
            if now - _lastFire < S.meleeDirectRate then return end
            _lastFire = now

            -- Encode cameradata payload (identical structure to source script)
            local u = _getUtil()
            if not u then return end
            local remote = _getRemote()
            if not remote then return end

            -- Enum token for UseItem
            local _ok, _token = pcall(function()
                return _enumLib:ToEnum("StartShooting")
            end)
            if not _ok or not _token then return end
            ...
            -- Small random sub-stud offset: breaks hitbox seam detection
            local rOff = Vector3.new(
                (math.random() - 0.5) * 0.1,
                (math.random() - 0.5) * 0.1,
                (math.random() - 0.5) * 0.1
            )
            local aimedPos       = targetPos + rOff
            local objSpaceOffset = targetHead.CFrame:ToObjectSpace(CFrame.new(aimedPos))

            local cameradata = {}
            cameradata[utf8.char(1)] = {
                [utf8.char(0)] = u:EncodeCFrame(aimCF),
                [utf8.char(1)] = u:EncodeCFrame(targetCF),
                [utf8.char(2)] = targetHead,
                [utf8.char(3)] = u:EncodeCFrame(objSpaceOffset)
            }

            -- Fire through arch's hook-invisible native path when available,
            -- plain FireServer otherwise.
            pcall(function()
                local oid = item:Get("ObjectID")
                if not oid then return end
                if fireServerNative then
                    fireServerNative(remote, oid, _token, cameradata, nil)
                else
                    remote:FireServer(oid, _token, cameradata, nil)
                end
            end)
        end)
    end
    ...
    local function _stop()
        _active = false
        if _mainConn then _mainConn:Disconnect(); _mainConn = nil end
        RunService:UnbindFromRenderStep("__mdd_restore")
    end

    Players.PlayerRemoving:Connect(function(p) _deflecting[p] = nil end)

    return {
        setEnabled = function(on)
            _active = on
            if on then _start() else _stop() end
        end,
        isEnabled = function() return _active end,
    }
end)()
-- ── end Melee Direct Fire Module ──────────────────────────────────────────
...
function ApplyAllSettings()
    ...
    stopIrregularMove()
    stopTriggerbot()
    MeleeDirectModule.setEnabled(false)
    _G.StopAntiAimExt()
    ...
    if S.meleeDirectActive then MeleeDirectModule.setEnabled(true) end
...
local MeleeDirectGroup = Rage:Group("Melee Direct Fire")
MeleeDirectGroup:Toggle({
    Name = "Enabled",
    Tooltip = "Desync-based direct melee remote fire. Bypasses wall check server-side.",
    Risky = true,
    Callback = function(v)
        S.meleeDirectActive = v
        MeleeDirectModule.setEnabled(v)
    end
})
MeleeDirectGroup:Dropdown({
    Name = "Weapon Slot",
    Options = {"Primary", "Secondary", "Melee"},
    Default = "Melee",
    Tooltip = "Auto-equips this slot for firing. Melee = knife/sword etc.",
    Callback = function(v)
        S.meleeDirectSlot = v
    end
})
MeleeDirectGroup:Slider({
    Name = "Fire Rate (ms)",
    Min = 0,
    Max = 500,
    Default = 1,
    Unit = "ms",
    Decimals = 0,
    Tooltip = "Minimum ms between shots. 0 = every heartbeat (~60/s). Lower = more hits, higher ban risk.",
    Callback = function(v)
        S.meleeDirectRate = v / 1000
    end
})
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if S.meleeDirectActive then
        MeleeDirectModule.setEnabled(true)
    end
