...
-- ─── AutoUseItem (desync-fire with slot selection) ────────────────────────────
local AutoUseItemModule = (function()
    local _repS     = cloneref(game:GetService("ReplicatedStorage"))
    local _plrs     = cloneref(game:GetService("Players"))
    local _runS     = cloneref(game:GetService("RunService"))
    local _ws       = cloneref(game:GetService("Workspace"))
    local _lplr     = _plrs.LocalPlayer

    -- re-use already-required modules if available, else require fresh
    local function _safeRequire(path)
        local ok, res = pcall(function()
            return require(_lplr.PlayerScripts.Controllers[path])
        end)
        return ok and res or nil
    end

    local _util     = require(_repS.Modules.Utility)
    local _enum     = require(_repS.Modules.EnumLibrary)
    local _fightCtrl = _safeRequire("FighterController")
    local _specCtrl  = _safeRequire("SpectateController")

    local _slots = { Primary = 1, Secondary = 2, Melee = 3 }
    ...
    local _deflecting = {}
    _plrs.PlayerRemoving:Connect(function(p) _deflecting[p] = nil end)

    -- Track katana deflect state using the same FighterController.Objects
    local function _updateDeflection()
        if not _fightCtrl or not _fightCtrl.Objects then return end
        for _, fighterObj in _fightCtrl.Objects do
            local player = fighterObj.Player
            if not player then continue end
            if not fighterObj.Entity or not fighterObj.Entity:IsAlive()
               or fighterObj:Get("IsSpectating") then
                _deflecting[player] = false
                continue
            end
            local equipped = fighterObj.EquippedItem
            local isKatana = equipped and equipped.ViewModel
                and equipped.ViewModel.Name == "Katana"
            local isDeflecting = false
            if isKatana then
                isDeflecting = (equipped._attack_cooldown
                    and equipped._attack_cooldown > tick()) or false
            end
            _deflecting[player] = isDeflecting
        end
    end

    local function _isEnemy(player)
        if player == _lplr then return false end
        local duel = _specCtrl and _specCtrl.CurrentDuelSubject
        local localDueler = duel and duel:GetDueler(_lplr)
        ...
        local lTeam = _lplr:GetAttribute("TeamID")
        if pTeam and lTeam then return pTeam ~= lTeam end
        return true
    end

    local function _getClosestTarget()
        ...
        for _, player in _plrs:GetPlayers() do
            if not _isEnemy(player) then continue end
    ...
    local function _hasKnifeViewModel(targetPlayer)
        if not targetPlayer then return false end
        local viewModels = _ws:FindFirstChild("ViewModels")
        if not viewModels then return false end
        for _, model in viewModels:GetChildren() do
            if model:IsA("Model")
               and string.find(model.Name, targetPlayer.Name, 1, true)
    ...
    -- Equip on start + periodic re-equip every second
    local function _equipLoop()
        task.spawn(function()
            local lf = _fightCtrl and _fightCtrl.LocalFighter
            while not lf do
                task.wait(0.1)
                lf = _fightCtrl and _fightCtrl.LocalFighter
            end
            pcall(function() lf:EquipItem(_slots[_state.weaponSlot] or 3) end)
        end)
    end

    local _equipConn = nil
    local function _startEquipLoop()
        if _equipConn then _equipConn:Disconnect() end
        _equipConn = _runS.Heartbeat:Connect(function()
            -- re-equip every ~60 frames (≈1 s)
        ...
        -- Throttle: only call EquipItem once per second using a flag
        -- We replace with a task.spawn loop to avoid every-frame equip calls
        if _equipConn then _equipConn:Disconnect(); _equipConn = nil end
        _equipConn = task.spawn(function()
            while _state.enabled do
                if _fightCtrl and _fightCtrl.LocalFighter then
                    pcall(function()
                        _fightCtrl.LocalFighter:EquipItem(_slots[_state.weaponSlot] or 3)
                    end)
                end
                task.wait(1)
            end
    ...
    local function _startFire()
        if _fireConn then task.cancel(_fireConn) end
        _fireConn = _runS.Heartbeat:Connect(function()
            _updateDeflection()
            if not _state.enabled then return end
            if not _fightCtrl or not _fightCtrl.LocalFighter then return end

            local targetPlayer, targetRoot, targetHead = _getClosestTarget()
            ...
            local item = _fightCtrl.LocalFighter.EquippedItem
            if not item then return end
            if tick() - _lastFire < _state.fireRate then return end
            _lastFire = tick()

            -- desync origin: above root, or above+offset for knife carrier
            local desyncPos
            if _hasKnifeViewModel(targetPlayer) then
                desyncPos = (targetRoot.CFrame * CFrame.new(0, 6, 0)).Position
            else
                desyncPos = (targetRoot.CFrame * CFrame.new(0, 1, 2)).Position
            end
            local originCF = CFrame.lookAt(desyncPos, targetHead.Position)
            local targetCF = targetHead.CFrame

            -- Apply desync then restore next render step
            local myRoot = char.HumanoidRootPart
            local oldCF  = myRoot.CFrame
            local oldVel = myRoot.Velocity
            local oldRV  = myRoot.RotVelocity
            myRoot.CFrame = originCF
            _runS:BindToRenderStep("__aui_restore", 101, function()
                if myRoot then
                    myRoot.CFrame = oldCF
                    myRoot.Velocity = oldVel
                    myRoot.RotVelocity = oldRV
                end
                _runS:UnbindFromRenderStep("__aui_restore")
            end)

            -- Build cameradata payload
            local randomOffset = Vector3.new(
                (math.random() - 0.5) * 0.1,
                (math.random() - 0.5) * 0.1,
                (math.random() - 0.5) * 0.1
            )
            local aimedPos = targetHead.Position + randomOffset
            local objSpaceOffset = targetHead.CFrame:ToObjectSpace(CFrame.new(aimedPos))

            local cameradata = {}
            cameradata[utf8.char(1)] = {
                [utf8.char(0)] = _util:EncodeCFrame(originCF),
                [utf8.char(1)] = _util:EncodeCFrame(targetCF),
                [utf8.char(2)] = targetHead,
                [utf8.char(3)] = _util:EncodeCFrame(objSpaceOffset)
            }

            local useItemRemote = _repS.Remotes.Replication.Fighter.UseItem
            useItemRemote:FireServer(
                item:Get("ObjectID"),
                _enum:ToEnum("StartShooting"),
                cameradata,
                nil
            )
        end)
    ...
    local function _stop()
        ...
        if type(_equipConn) == "thread" then
            pcall(task.cancel, _equipConn)
        elseif _equipConn and _equipConn.Disconnect then
            _equipConn:Disconnect()
        end
        _equipConn = nil
        if _fireConn then _fireConn:Disconnect(); _fireConn = nil end
    end
    ...
    return {
        ...
        SetSlot = function(slot)
            _state.weaponSlot = slot
            -- re-equip immediately on slot change
            if _state.enabled and _fightCtrl and _fightCtrl.LocalFighter then
                pcall(function()
                    _fightCtrl.LocalFighter:EquipItem(_slots[slot] or 3)
                end)
            end
        end,
        SetFireRate = function(ms)
            _state.fireRate = ms / 1000
        end,
        IsEnabled = function() return _state.enabled end,
    }
end)()

-- ─── Rage Tab UI — AutoUseItem group (bottom of Rage) ────────────────────────
local AutoUseGroup = Rage:Group("AutoUseItem")
AutoUseGroup:Toggle({
    Name = "Enabled",
    Tooltip = "Desync-fire using selected weapon slot (WeaponSlot chooser)",
    Callback = function(v)
        AutoUseItemModule.SetEnabled(v)
    end
})
AutoUseGroup:Dropdown({
    Name = "Weapon Slot",
    Options = {"Primary", "Secondary", "Melee"},
    Default = "Melee",
    Tooltip = "Which slot to equip and use",
    Callback = function(v)
        AutoUseItemModule.SetSlot(v)
    end
})
AutoUseGroup:Slider({
    Name = "Fire Rate",
    ...
    Default = 0,
    Unit = "ms",
    Tooltip = "Minimum ms between shots (0 = max speed)",
    Callback = function(v)
        AutoUseItemModule.SetFireRate(v)
    end
})
