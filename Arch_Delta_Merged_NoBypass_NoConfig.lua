-- ============================================================
-- Arch Scripts + Ragebot/Flickbot | Delta-compatible merged build
-- Delta-compatible merged build; executor APIs are feature-gated.
-- ============================================================
local _Genv = (type(getgenv) == "function" and getgenv) or function() return _G end
if type(getgenv) ~= "function" then function getgenv() return _G end end
local _clonefunction = (type(clonefunction) == "function" and clonefunction)
    or (type(copyfunction) == "function" and copyfunction)
    or function(f) return f end
local _cloneref = (type(cloneref) == "function" and cloneref)
    or (type(clonereference) == "function" and clonereference)
    or function(v) return v end
local _sethiddenproperty = (type(sethiddenproperty) == "function" and sethiddenproperty)
    or function(obj, prop, value)
        if obj ~= nil then pcall(function() obj[prop] = value end) end
    end
local _getthreadidentity = (type(getthreadidentity) == "function" and getthreadidentity)
    or function() return 0 end
local _setthreadidentity = (type(setthreadidentity) == "function" and setthreadidentity)
    or function() end
...
-- ===== Merged Ragebot/Flickbot settings =====
local Settings = {
    Enabled = false, Stability = 0.15, ShootFrames = 1, PrioritizeHackers = false,
    WeaponPrimary = true, WeaponSecondary = true, WeaponMelee = true,
    OnEmpty = "SwapOrReload", EvasionMode = "Random", TranslocateOffset = -5,
    RandomBaseRadius = 100, RandomRadiusFactor = 0.5, RandomAnchorFromCharacter = false,
    FlickbotEnabled = false, FlickbotKeybind = Enum.KeyCode.Unknown, FlickbotShot = false,
    FlickbotShotDelay = 0, FlickbotCooldown = 250, FlickbotDuration = 110,
    FlickbotCurvature = 12, FlickbotHumanness = 30,
}
...
pcall(function() local old = CoreGui:FindFirstChild("ArchScriptsUI"); if old then old:Destroy() end end)
...
-- Rage core uses a local notifier; route it through the existing UI notifier.
local function Notify(msg, kind)
    if Library and type(Library.Notify) == "function" then
        pcall(Library.Notify, Library, tostring(msg), kind)
    end
end
...
-- ============================================================
-- Rage+ page: intentionally placed immediately above Config.
-- ============================================================
local DeltaTab = Library:Tab("Rage+", 10455604811)
local DeltaRageGroup = DeltaTab:Group("Ragebot")
DeltaRageGroup:Toggle({Name="Enabled", Callback=function(v) Settings.Enabled=v end})
DeltaRageGroup:Slider({Name="Stability", Min=0, Max=1.5, Default=Settings.Stability, Callback=function(v) Settings.Stability=v end})
DeltaRageGroup:Slider({Name="Shoot Frames", Min=1, Max=5, Default=Settings.ShootFrames, Unit=" frames", Callback=function(v) Settings.ShootFrames=v end})
DeltaRageGroup:Toggle({Name="Prioritize Hackers", Callback=function(v) Settings.PrioritizeHackers=v end})
DeltaRageGroup:Toggle({Name="Use Primary", Default=true, Callback=function(v) Settings.WeaponPrimary=v end})
DeltaRageGroup:Toggle({Name="Use Secondary", Default=true, Callback=function(v) Settings.WeaponSecondary=v end})
DeltaRageGroup:Toggle({Name="Use Melee", Default=true, Callback=function(v) Settings.WeaponMelee=v end})
DeltaRageGroup:Dropdown({Name="On Empty", Options={"Swap","Reload","SwapOrReload"}, Default=Settings.OnEmpty, Callback=function(v) Settings.OnEmpty=v end})
DeltaRageGroup:Dropdown({Name="Evasion Mode", Options={"Random","Translocate","ProjectileBreaker"}, Default=Settings.EvasionMode, Callback=function(v) Settings.EvasionMode=v end})
DeltaRageGroup:Slider({Name="Translocate Offset", Min=-5, Max=5, Default=Settings.TranslocateOffset, Callback=function(v) Settings.TranslocateOffset=v end})
DeltaRageGroup:Slider({Name="Random Base Radius", Min=5, Max=100000, Default=Settings.RandomBaseRadius, Callback=function(v) Settings.RandomBaseRadius=v end})
DeltaRageGroup:Slider({Name="Random Range Factor", Min=0, Max=1, Default=Settings.RandomRadiusFactor, Decimals=2, Callback=function(v) Settings.RandomRadiusFactor=v end})
DeltaRageGroup:Toggle({Name="Character Origin", Callback=function(v) Settings.RandomAnchorFromCharacter=v end})

local DeltaFlickGroup = DeltaTab:Group("Flickbot")
DeltaFlickGroup:Toggle({Name="Enabled", Callback=function(v) Settings.FlickbotEnabled=v end})
DeltaFlickGroup:Keybind({Name="Keybind", Default=Enum.KeyCode.Unknown, Callback=function(k) Settings.FlickbotKeybind=k end})
DeltaFlickGroup:Toggle({Name="Auto Shoot", Callback=function(v) Settings.FlickbotShot=v end})
DeltaFlickGroup:Slider({Name="Shot Delay", Min=0, Max=250, Default=Settings.FlickbotShotDelay, Unit=" ms", Callback=function(v) Settings.FlickbotShotDelay=v end})
DeltaFlickGroup:Slider({Name="Cooldown", Min=0, Max=2000, Default=Settings.FlickbotCooldown, Unit=" ms", Callback=function(v) Settings.FlickbotCooldown=v end})
DeltaFlickGroup:Slider({Name="Flick Duration", Min=30, Max=400, Default=Settings.FlickbotDuration, Unit=" ms", Callback=function(v) Settings.FlickbotDuration=v end})
DeltaFlickGroup:Slider({Name="Curvature", Min=0, Max=50, Default=Settings.FlickbotCurvature, Callback=function(v) Settings.FlickbotCurvature=v end})
DeltaFlickGroup:Slider({Name="Humanness", Min=0, Max=100, Default=Settings.FlickbotHumanness, Unit="%", Callback=function(v) Settings.FlickbotHumanness=v end})

local DeltaDiagGroup = DeltaTab:Group("Delta Compatibility")
local function DeltaStatus(fn)
    local ok, value = pcall(fn)
    return ok and value ~= nil
end
DeltaDiagGroup:Button({Name="Run Compatibility Check", Variant="Primary", Callback=function()
    local checks={
        {"clonefunction/copyfunction", DeltaStatus(function() return _clonefunction end)},
        {"sethiddenproperty", DeltaStatus(function() return _sethiddenproperty end)},
        {"getthreadidentity", DeltaStatus(function() return _getthreadidentity end)},
        {"setthreadidentity", DeltaStatus(function() return _setthreadidentity end)},
        {"getgenv", DeltaStatus(function() return _Genv end)},
        {"readfile", type(readfile)=="function"}, {"writefile", type(writefile)=="function"}, {"isfile", type(isfile)=="function"},
    }
    for _,item in ipairs(checks) do Notify(item[1].." : "..(item[2] and "AVAILABLE" or "MISSING"), item[2] and "success" or "warning") end
end})
DeltaDiagGroup:Button({Name="Reset Ragebot", Callback=function() if type(ResetRagebot)=="function" then pcall(ResetRagebot) end end})
...
Library:Notify("Arch Scripts + Mobile ESP + Advanced Anti-Aim/Underground + Anti Katana + Slide Boost + Wallbang Head + Config loaded", "success")

-- ============================================================
-- Merged Ragebot/Flickbot core
-- ============================================================
-- 解析枚举库
...
-- ===== Kicia 风格远程射击 (克隆 FireServer + 虚空瞄准) =====
local NEG_HUGE = -9e37
...
function RootDesync.new(rootPart)
    ...
    pcall(RunService.UnbindFromRenderStep, RunService, "KiciaRootDesync")
...
-- ===== Part Glue (使用 sethiddenproperty) =====
local PartGlue = {}
...
function ViewAngleDriver.new()
    return setmetatable({ _slots = {}, _winning = nil }, ViewAngleDriver)
end
function ViewAngleDriver:SendViewAngles(priority, angles)
    if not angles then self._slots[priority] = nil else self._slots[priority] = angles end
    local best = nil
    for pri, val in pairs(self._slots) do
        if best == nil or pri > best then best = pri end
    end
    self._winning = self._slots[best]
end
function ViewAngleDriver:Flush()
    local winning = self._winning
    if winning then
        local remote = ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("Replication") and ReplicatedStorage.Remotes.Replication:FindFirstChild("Fighter") and ReplicatedStorage.Remotes.Replication.Fighter:FindFirstChild("UpdateCameraRotation")
        if remote then
            local pitch, yaw = winning.X, winning.Y
            local enc = utf8.char(math.clamp(math.floor(pitch / math.pi / 2 * 256), 0, 255)) .. utf8.char(math.clamp(math.floor(yaw / math.pi / 2 * 256), 0, 255))
            pcall(remote.FireServer, remote, enc, nil)
        end
    end
end
    ...
    if action and action.type == "Attack" then
        ...
        if objId and target then
            ...
            elseif item.Info and (item.Info.Class == "Melee" or item.Info.Type == "Melee") then
                local aim1 = buildAim(AIM_ABOVE_ORIGIN, PITCH_ABOVE, 0, 0)
                local aim2 = buildAim(AIM_ABOVE_END, PITCH_ABOVE, 0, 0)
                fireMeleeAttack(objId, aim1, aim2, target.hitboxHead, AIM_EXTRA)
            end
        end
...
local _erf = (type(math.erf) == "function" and math.erf) or function(x)
...
function GenerateFlickbotTrajectory(startX, startY, targetX, targetY)
...
function UpdateFlickbot(dt)
    ...
    if Flickbot.State == "Flicking" then
        ...
        local cameraForDir = Workspace.CurrentCamera
        if not cameraForDir then Flickbot.State = "Idle"; return end
        local rayA = cameraForDir:ViewportPointToRay(current.x, current.y)
        local rayB = cameraForDir:ViewportPointToRay(nxt.x, nxt.y)
        local dir = rayA.Direction:Lerp(rayB.Direction, alpha)
        if dir.Magnitude < 1e-6 then Flickbot.State = "Idle"; return end
        dir = dir.Unit
...
task.delay(1, function()
    if game.GameId == 6035872082 then
        Notify("Kicia Ragebot 完整版已加载 (含 Desync + PartGlue)", "success")
        RunService.Heartbeat:Connect(OnHeartbeat)
    else
        Notify("当前游戏不是 RIVALS，Ragebot 不会工作", "warning")
    end
end)

Notify("脚本加载完成，按 Insert 打开菜单", "success")
Notify("Ragebot 默认禁用，请在菜单中启用", "warning")
