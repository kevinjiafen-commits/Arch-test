...
local function updateMobileESP()
    ...
    if not myChar then hideAllESP(); clearAllChams(); return end
    local myRoot = myChar:FindFirstChild("HumanoidRootPart")
    if not myRoot then hideAllESP(); clearAllChams(); return end
...
ESPGroup:Toggle({Name = "Chams", Tooltip = "Red chams through walls", Callback = function(v) S.espShowChams = v; if not v then clearAllChams() end end})
