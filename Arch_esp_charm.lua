...
local S = {
    ...
    espShowCharm = true,
    espCharmColor = Color3.new(1, 0, 0),
...
for i = 1, MAX_ESP do
    ...
    -- Charm indicator (red label shown below distance)
    local charmLabel = Instance.new("TextLabel")
    charmLabel.BackgroundTransparency = 1
    charmLabel.TextSize = 13
    charmLabel.Font = Enum.Font.GothamBold
    charmLabel.TextColor3 = Color3.new(1, 0, 0)
    charmLabel.Text = ""
    charmLabel.Visible = false
    charmLabel.Parent = box

    box.Parent = espGui
    table.insert(espPool, {box = box, outline = outline, fill = fill, name = name, dist = dist, hpBar = hpBar, hpFill = hpFill, charmLabel = charmLabel})
end

local function hideAllESP()
    for _, esp in ipairs(espPool) do
        esp.box.Visible = false
        if esp.charmLabel then esp.charmLabel.Visible = false end
    end
end

-- Detect equipped charm cosmetic on a player using CosmeticLibrary
local function getPlayerCharmName(player)
    -- Method 1: check via PlayerDataController weapon data
    local ok1, wepData = pcall(function()
        return _datCtrl:GetWeaponData(player.Name)
    end)
    if ok1 and type(wepData) == "table" then
        local charm = wepData.Charm
        if charm and charm.Name and charm.Name ~= "" then
            return charm.Name
        end
    end

    -- Method 2: iterate equipped table (_eq) for local player check
    if player == LocalPlayer then
        for wn, cos in pairs(_eq) do
            if cos.Charm and cos.Charm.Name then
                return cos.Charm.Name
            end
        end
    end

    -- Method 3: scan FighterController for replicated charm data
    pcall(function()
        local fc = FighterController
        if fc then
            local fighter = fc:GetFighter(player)
            if fighter then
...
local function updateMobileESP()
    ...
    for _, enemy in ipairs(enemies) do
        ...
        if onScreen then
            ...
            -- Charm display (red label)
            if S.espShowCharm and esp.charmLabel then
                local charmName = getPlayerCharmName(enemy.Player)
                if charmName then
                    local vertOffset = S.espShowDistance and 28 or 10
                    esp.charmLabel.Text = "✦ " .. charmName
                    esp.charmLabel.TextColor3 = S.espCharmColor
                    esp.charmLabel.Position = UDim2.new(0.5, 0, 1, vertOffset)
                    esp.charmLabel.Size = UDim2.new(1, 20, 0, 14)
                    esp.charmLabel.AnchorPoint = Vector2.new(0.5, 0)
                    esp.charmLabel.TextXAlignment = Enum.TextXAlignment.Center
                    esp.charmLabel.Visible = true
                else
                    esp.charmLabel.Visible = false
                end
            else
                if esp.charmLabel then esp.charmLabel.Visible = false end
            end
...
ESPGroup:Toggle({Name = "Charm", Default = true, Tooltip = "Show equipped charm name in red on ESP box", Callback = function(v) S.espShowCharm = v end})
ESPGroup:ColorPicker({Name = "Charm Color", Default = Color3.new(1, 0, 0), Tooltip = "Color of the charm label (default red)", Callback = function(c) S.espCharmColor = c end})
