...
-- Supports nested tables, Color3 and EnumItem values while skipping
-- functions, connections, Instances and userdata that JSON cannot encode.
local function _serializeConfigValue(value, seen)
    ...
    for k, v in pairs(value) do
        local serialized = _serializeConfigValue(v, seen)
...
local function _deserializeConfigValue(value)
    if type(value) ~= "table" then
        return value
    end
    ...
    if value.__type == "Enum" then
        ...
        if type(enumName) == "string" and type(itemName) == "string" then
    ...
    local result = {}
    for k, v in pairs(value) do
        local decoded = _deserializeConfigValue(v)
        if decoded ~= nil then
            result[k] = decoded
        end
    end
...
local function _mergeConfigTable(target, source)
...
local function saveConfig()
    ...
    if type(S) == "table" then
        data.S = _serializeConfigValue(S) or {}
    else
        data.S = {}
    end

    -- 2. Anti-Aim settings.
    if type(_G.AntiAimSettings) == "table" then
        data.AntiAimSettings = _serializeConfigValue(_G.AntiAimSettings) or {}
    end

    -- 3. Ragebot / Rage+ / Flickbot settings.
    if RageCore and type(RageCore.Settings) == "table" then
        data.RageSettings = _serializeConfigValue(RageCore.Settings) or {}
    elseif type(RageSettings) == "table" then
        data.RageSettings = _serializeConfigValue(RageSettings) or {}
    end

    -- 4. Slide Boost settings.
    if _G.Features and type(_G.Features.SlideBoost) == "table" then
        data.SlideBoost = _serializeConfigValue(_G.Features.SlideBoost) or {}
    end

    -- 5. Write the complete configuration.
    local ok, json = pcall(function()
        return HttpService:JSONEncode(data)
    end)

    if not ok or not json then
        Library:Notify("配置编码失败", "warning")
        return
    end

    if writefile then
        local wrote = pcall(writefile, configName, json)
        if wrote then
            Library:Notify("所有配置已保存", "success")
        else
            Library:Notify("配置保存失败", "warning")
        end
    else
...
local function loadConfig(silent)
    ...
    local existsOk, exists = pcall(isfile, configName)
    if not existsOk or not exists then
        if not silent then Library:Notify("配置文件不存在", "warning") end
        return
    end

    local readOk, raw = pcall(readfile, configName)
    if not readOk or type(raw) ~= "string" or raw == "" then
        if not silent then Library:Notify("配置读取失败", "warning") end
        return
    end
    ...
    -- Restore main S settings.
    for k, v in pairs(sourceS) do
        if k ~= "AntiAimSettings"
            and k ~= "RageSettings"
            and k ~= "SlideBoost"
            and k ~= "S" then
    ...
    -- Restore Anti-Aim.
    if type(data.AntiAimSettings) == "table" then
        _G.AntiAimSettings = _G.AntiAimSettings or {}
        _mergeConfigTable(_G.AntiAimSettings, data.AntiAimSettings)
    end

    -- Restore Ragebot / Rage+ / Flickbot.
    local rageTarget = nil
    if RageCore and type(RageCore.Settings) == "table" then
        rageTarget = RageCore.Settings
    elseif type(RageSettings) == "table" then
        rageTarget = RageSettings
    end

    if type(data.RageSettings) == "table" and rageTarget then
        _mergeConfigTable(rageTarget, data.RageSettings)
    end

    -- Restore Slide Boost.
    if type(data.SlideBoost) == "table" then
        _G.Features = _G.Features or {}
        _G.Features.SlideBoost = _G.Features.SlideBoost or {}
        _mergeConfigTable(_G.Features.SlideBoost, data.SlideBoost)
    end

    -- Re-apply the main S settings first.
    pcall(function()
        ApplyAllSettings()
    end)

    -- Re-apply Anti-Aim after loading its parameters.
    pcall(function()
        if _G.AntiAimSettings and _G.AntiAimSettings.enabled then
            if type(_G.StartAntiAimExt) == "function" then
                _G.StartAntiAimExt()
            end
        elseif type(_G.StopAntiAimExt) == "function" then
            _G.StopAntiAimExt()
        end
    end)

    -- Re-apply Slide Boost after loading its parameters.
    pcall(function()
        local sb = _G.Features and _G.Features.SlideBoost
        if sb and SlideBoostModule and type(SlideBoostModule.setSlideBoost) == "function" then
            SlideBoostModule.setSlideBoost(sb.Enabled == true, sb.Speed)
        end
    end)

    -- RageCore reads its Settings table continuously, so restoring the
    -- table values is sufficient; reset transient state when available.
    pcall(function()
        if RageCore and type(RageCore.ResetRagebot) == "function" then
            RageCore.ResetRagebot()
        end
    end)

    if not silent then
        Library:Notify("全部配置已加载", "success")
    end
end
