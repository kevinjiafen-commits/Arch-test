...
--==================================================
-- EMOTE ENGINE (extracted from Eclipse Emote UI)
--==================================================

local EMOTE_PRESET_ORDER = {
...
local emoteActiveTracks = {}

local function emote_getAnimator(char)
...
local function emote_resolveAnim(id)
...
local function emote_loadPlay(char, id, speed, looped)
    if not char or type(id) ~= "string" or id == "" then return nil end

    local animator = emote_getAnimator(char)
    if not animator then return nil end

    local animation = emote_resolveAnim(id)
    ...
    pcall(function()
        track:Play(0.1, 1, tonumber(speed) or 1)
    end)

    return track
end

local function emote_stopAll()
    for _, track in ipairs(emoteActiveTracks) do
        if track then
            pcall(function()
                track:Stop(0)
                track:Destroy()
    ...
    table.clear(emoteActiveTracks)
end

local function emote_play()
    emote_stopAll()
    ...
    local char = LocalPlayer.Character
    if char then
        local track = emote_loadPlay(char, emoteState.animationId, emoteState.speed, emoteState.loop)
        if track then table.insert(emoteActiveTracks, track) end
    end
    ...
    if live then
        ...
        if sc then
            local track = emote_loadPlay(sc, emoteState.animationId, emoteState.speed, emoteState.loop)
            if track then table.insert(emoteActiveTracks, track) end
        end
    end
end

-- Heartbeat: keep animation alive and adjust speed live
RunService.Heartbeat:Connect(function()
    if not emoteState.enabled then return end

    if #emoteActiveTracks == 0 and emoteState.animationId ~= "" then
        emote_play()
        return
    end
    ...
    for _, track in ipairs(emoteActiveTracks) do
        if track then
            pcall(function()
                track:AdjustSpeed(emoteState.speed)
            end)
        end
    end
end)

-- SpawnProof: replay emote on character respawn
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    if emoteState.enabled and emoteState.spawnProof then
        emote_play()
    end
end)
...
--==================================================
-- EMOTE UI (injected into Settings tab, right column)
--==================================================

-- Preset group — right column of Config tab
local EmotePresetGroup = ConfigTab:Group("Emote Presets")

local emote_activePresetBtn = nil
local emote_presetButtons = {}

local function emote_clearPreset()
    if emote_activePresetBtn then
        -- visual reset handled per-button below
        emote_activePresetBtn = nil
    end
end

for _, presetName in ipairs(EMOTE_PRESET_ORDER) do
    local capturedName = presetName
    EmotePresetGroup:Button({
        Name = capturedName,
        Callback = function()
            -- toggle off if same preset clicked again
            if emoteState.enabled and emoteState.presetName == capturedName then
                emoteState.enabled    = false
                emoteState.animationId = ""
                emoteState.presetName  = nil
                emote_stopAll()
                emote_clearPreset()
                Library:Notify("Emote stopped", "success")
                return
            end
            ...
            emoteState.presetName  = capturedName
            emoteState.animationId = emoteAnimPresets[capturedName] or ""
            emote_activePresetBtn  = capturedName

            emote_play()
            Library:Notify("Playing: " .. capturedName, "success")
        end,
    })
end

-- Custom emote group — right column
local EmoteCustomGroup = ConfigTab:Group("Emote Custom")

local emoteCustomIDBox = EmoteCustomGroup:Textbox({
    Name        = "Animation ID",
    Placeholder = "Asset ID...",
    Default     = "",
})

EmoteCustomGroup:Button({
    Name     = "Play Custom",
    Variant  = "Primary",
    Callback = function()
        local raw = emoteCustomIDBox and emoteCustomIDBox:Get() or ""
        local id  = tostring(raw):match("%d+")

        if not id then
            Library:Notify("Invalid animation ID", "warning")
            return
        end
        ...
        emote_play()
        Library:Notify("Custom emote started", "success")
    end,
})

-- Options group — right column
local EmoteOptionsGroup = ConfigTab:Group("Emote Options")

EmoteOptionsGroup:Slider({
    Name     = "Speed",
    Min      = 1,
    Max      = 500,
    Default  = emoteState.speed,
    Callback = function(value)
        emoteState.speed = math.max(1, tonumber(value) or 1)
    end,
})

EmoteOptionsGroup:Toggle({
    Name     = "Loop",
    Default  = emoteState.loop,
    Callback = function(value)
        emoteState.loop = value == true
        if emoteState.enabled then
            emote_play()
        end
    end,
})
...
EmoteOptionsGroup:Toggle({
    Name     = "SpawnProof",
    Default  = emoteState.spawnProof,
    Callback = function(value)
        emoteState.spawnProof = value == true
    end,
})

EmoteOptionsGroup:Button({
    Name     = "Stop Emote",
    Variant  = "Danger",
    Callback = function()
        emoteState.enabled     = false
