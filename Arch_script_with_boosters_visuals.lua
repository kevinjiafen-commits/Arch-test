...
-- Booster collection module
local BoosterSettings = {
    SupportedCreatorId = 3461453,
    AutoHealth = true,
    AutoAmmo = true,
    AutoRespawn = true,
}
...
local function BoosterGameSupported()
    return game.CreatorId == BoosterSettings.SupportedCreatorId
end

local function StopBoosterRespawnLoop()
    BoosterState.Running = false
end

local function StartBoosterRespawnLoop()
    if not BoosterSettings.AutoRespawn or not BoosterGameSupported() then
        return
    end
    if BoosterState.Running then
        return
    end
    BoosterState.Running = true
    task.spawn(function()
        while BoosterState.Running and BoosterSettings.AutoRespawn do
            local character = LocalPlayer.Character
...
local function BindBoosterCharacter(character)
    if BoosterState.DeathConnection then
        pcall(function() BoosterState.DeathConnection:Disconnect() end)
        BoosterState.DeathConnection = nil
    end
    ...
    if humanoid then
        BoosterState.DeathConnection = humanoid.Died:Connect(function()
            StartBoosterRespawnLoop()
        end)
    end
end

local function StartBoosterCollector()
    if BoosterState.Connection then
        BoosterState.Connection:Disconnect()
        BoosterState.Connection = nil
    end
    BoosterState.Connection = RunService.RenderStepped:Connect(function(dt)
        if not BoosterGameSupported() then
            return
        end
        local character = LocalPlayer.Character
        ...
        local bounce = math.sin(os.clock() * 8) * 4
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj and obj.Name == "_drop" and obj:IsA("BasePart") then
                local isAmmo = BoosterSettings.AutoAmmo and obj:FindFirstChild("Ammo") ~= nil
                local isHealth = BoosterSettings.AutoHealth and obj:FindFirstChild("Health") ~= nil
                if isAmmo or isHealth then
                    pcall(function()
                        obj.Anchored = true
                        ...
                        for _, child in ipairs(obj:GetDescendants()) do
                            ...
                            elseif child:IsA("BillboardGui") or child:IsA("SurfaceGui") or child:IsA("ParticleEmitter") or child:IsA("SelectionBox") then
...
local function StopBoosterCollector()
    if BoosterState.Connection then
        BoosterState.Connection:Disconnect()
        BoosterState.Connection = nil
    end
    StopBoosterRespawnLoop()
end

BoosterState.CharacterConnection = LocalPlayer.CharacterAdded:Connect(function(character)
    BindBoosterCharacter(character)
end)
BindBoosterCharacter(LocalPlayer.Character)
StartBoosterCollector()

local BoosterGroup = Visuals:Group("Boosters")
BoosterGroup:Toggle({Name = "Auto Health", Default = true, Callback = function(v) BoosterSettings.AutoHealth = v end})
BoosterGroup:Toggle({Name = "Auto Ammo", Default = true, Callback = function(v) BoosterSettings.AutoAmmo = v end})
BoosterGroup:Toggle({Name = "Auto Respawn", Default = true, Callback = function(v)
    BoosterSettings.AutoRespawn = v
    if v then
        BindBoosterCharacter(LocalPlayer.Character)
    else
        StopBoosterRespawnLoop()
...
Library:Notify("Arch Scripts + Mobile ESP + Advanced Anti-Aim/Underground + Anti Katana + Slide Boost + Boosters + Config loaded", "success")
