...
-- ============================================================
-- KICIA RAGEBOT + FLICKBOT — INJECTED FROM arch_rage_.txt
-- ============================================================
local CollectionService = game:GetService("CollectionService")
local SoundService      = game:GetService("SoundService")
local TeleportService   = game:GetService("TeleportService")
local TextService_Rage  = game:GetService("TextService")

local cloneref          = cloneref or clonereference or function(i) return i end
local clonefunction_r   = clonefunction or copyfunction or function(f) return f end
local sethiddenproperty = sethiddenproperty or function(obj, prop, val) pcall(function() obj[prop] = val end) end
...
-- Ragebot settings table (separate from S to avoid conflicts)
local RageSettings = {
    Enabled = false,
    Stability = 0.15,
    ShootFrames = 1,
...
-- 核心 Ragebot + Flickbot (完整移植自 Kicia)
-- ============================================================

-- 解析枚举库
local RageEnumLib = nil
local function ResolveEnumLibrary()
    if RageEnumLib then return RageEnumLib end
...
-- 解析本地 Fighter
local RageFighterCtrl = nil
local function ResolveFighterController()
    if RageFighterCtrl then return RageFighterCtrl end
...
local function SelectTarget()
    ...
    local prioritizeHackers = RageSettings.PrioritizeHackers
...
local function isCategoryEnabled(cat)
    if cat == "Primary" then return RageSettings.WeaponPrimary
    elseif cat == "Secondary" then return RageSettings.WeaponSecondary
    elseif cat == "Melee" then return RageSettings.WeaponMelee
    end
    return false
end
...
local function getAction(fighter)
    ...
    local onEmpty = RageSettings.OnEmpty
...
local function RandomEvade(clientCF)
    local base = RageSettings.RandomBaseRadius
    local factor = RageSettings.RandomRadiusFactor
    local angle = math.random() * 2 * math.pi
    local r = base + math.random() * base * factor
    local anchor = RageSettings.RandomAnchorFromCharacter and clientCF.Position or Vector3.new(0, clientCF.Position.Y, 0)
...
local function TranslocateEvade()
    ...
    if chosen then
        return chosen.CFrame * CFrame.new(0, -chosen.Size.Y/2 + RageSettings.TranslocateOffset, 0)
    else
...
function Controller:Update(dt)
    ...
    if target and action then
        ...
        local mode = RageSettings.EvasionMode
        local evCF
        if mode == "Random" then
...
function UpdateRagebot(dt)
    ...
    local enabled = RageSettings.Enabled
...
function GenerateFlickbotTrajectory(startX, startY, targetX, targetY)
    ...
    local duration = RageSettings.FlickbotDuration
    local humanness = RageSettings.FlickbotHumanness / 30
    local curvScale = RageSettings.FlickbotCurvature / 1000
...
function UpdateFlickbot(dt)
    local enabled = RageSettings.FlickbotEnabled
    if not enabled then
        Flickbot.State = "Idle"
        return
    end
    local key = RageSettings.FlickbotKeybind
    ...
    if Flickbot.State == "Flicking" then
        ...
        if not nxt then
            if RageSettings.FlickbotShot then
                local delay = RageSettings.FlickbotShotDelay / 1000
                if delay > 0 then
...
-- ── NEW: Ragebot Tab (Kicia Ragebot + Flickbot) ─────────────────────────────
local RagebotTab = Library:Tab("Ragebot", 10455604811)

local RbGroup = RagebotTab:Group("Ragebot")
RbGroup:Toggle({Name = "Enabled", Tooltip = "Enable Kicia Ragebot", Callback = function(v) RageSettings.Enabled = v end})
RbGroup:Slider({Name = "Stability", Min = 0, Max = 1.5, Default = 0.15, Decimals = 2, Callback = function(v) RageSettings.Stability = v end})
RbGroup:Slider({Name = "Shoot Frames", Min = 1, Max = 5, Default = 1, Unit = "f", Callback = function(v) RageSettings.ShootFrames = v end})
RbGroup:Toggle({Name = "Prioritize Hackers", Callback = function(v) RageSettings.PrioritizeHackers = v end})
RbGroup:Toggle({Name = "Use Primary", Callback = function(v) RageSettings.WeaponPrimary = v end})
RbGroup:Toggle({Name = "Use Secondary", Callback = function(v) RageSettings.WeaponSecondary = v end})
RbGroup:Toggle({Name = "Use Melee", Callback = function(v) RageSettings.WeaponMelee = v end})
RbGroup:Dropdown({Name = "On Empty", Options = {"Swap","Reload","SwapOrReload"}, Default = "SwapOrReload", Callback = function(v) RageSettings.OnEmpty = v end})
RbGroup:Dropdown({Name = "Evasion Mode", Options = {"Random","Translocate","ProjectileBreaker"}, Default = "Random", Callback = function(v) RageSettings.EvasionMode = v end})
RbGroup:Slider({Name = "Translocate Offset", Min = -5, Max = 5, Default = -5, Callback = function(v) RageSettings.TranslocateOffset = v end})
RbGroup:Slider({Name = "Random Base Radius", Min = 5, Max = 500, Default = 100, Callback = function(v) RageSettings.RandomBaseRadius = v end})
RbGroup:Slider({Name = "Random Range Factor", Min = 0, Max = 1, Default = 0.5, Decimals = 2, Callback = function(v) RageSettings.RandomRadiusFactor = v end})
RbGroup:Toggle({Name = "Character Origin", Tooltip = "Anchor random evasion to character", Callback = function(v) RageSettings.RandomAnchorFromCharacter = v end})

local FlickGroup = RagebotTab:Group("Flickbot")
FlickGroup:Toggle({Name = "Enabled", Callback = function(v) RageSettings.FlickbotEnabled = v end})
FlickGroup:Keybind({Name = "Keybind", Default = Enum.KeyCode.Unknown, Callback = function(k) RageSettings.FlickbotKeybind = k end})
FlickGroup:Toggle({Name = "Auto Shoot", Callback = function(v) RageSettings.FlickbotShot = v end})
FlickGroup:Slider({Name = "Shot Delay", Min = 0, Max = 250, Default = 0, Unit = "ms", Callback = function(v) RageSettings.FlickbotShotDelay = v end})
FlickGroup:Slider({Name = "Cooldown", Min = 0, Max = 2000, Default = 250, Unit = "ms", Callback = function(v) RageSettings.FlickbotCooldown = v end})
FlickGroup:Slider({Name = "Flick Duration", Min = 30, Max = 400, Default = 110, Unit = "ms", Callback = function(v) RageSettings.FlickbotDuration = v end})
FlickGroup:Slider({Name = "Curvature", Min = 0, Max = 50, Default = 12, Callback = function(v) RageSettings.FlickbotCurvature = v end})
FlickGroup:Slider({Name = "Humanness", Min = 0, Max = 100, Default = 30, Unit = "%", Callback = function(v) RageSettings.FlickbotHumanness = v end})
...
-- ── Ragebot + Flickbot heartbeat loop ────────────────────────────────────────
task.delay(1, function()
    if game.GameId == 6035872082 then
        Library:Notify("Kicia Ragebot loaded (Desync + PartGlue)", "success")
        RunService.Heartbeat:Connect(function(dt)
            UpdateRagebot(dt)
            UpdateFlickbot(dt)
        end)
    else
        Library:Notify("Not RIVALS — Ragebot inactive", "warning")
    end
end)
