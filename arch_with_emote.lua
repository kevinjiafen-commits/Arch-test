...
-- ═══════════════════════════════════════════════════════════════
-- EMOTE ENGINE — integrated, no separate ScreenGui
-- ═══════════════════════════════════════════════════════════════
local _emRun     = game:GetService("RunService")
local _emPlayers = game:GetService("Players")
local _emLP      = _emPlayers.LocalPlayer
local _emWS      = game:GetService("Workspace")
...
local function _emGetAnimator(char)
...
local function _emLoadPlay(char, id, spd, loop)
    if not char or not id or id == "" then return nil end
    local animator = _emGetAnimator(char)
    if not animator then return nil end
    local obj = _emResolveAnim(id)
    if not obj then return nil end
    local ok, track = pcall(function() return animator:LoadAnimation(obj) end)
...
local function _emPlayEmote()
    _emStopAll()
    if not emoteState.enabled or emoteState.animationId == "" then return end
    local char = _emLP.Character
    if char then
        local t = _emLoadPlay(char, emoteState.animationId, emoteState.speed, emoteState.loop)
        if t then table.insert(_activeTracks, t) end
    end
    local Live = _emWS:FindFirstChild("Live")
    if Live then
        local sc = Live:FindFirstChild(_emLP.Name)
        if sc then
            local t = _emLoadPlay(sc, emoteState.animationId, emoteState.speed, emoteState.loop)
            if t then table.insert(_activeTracks, t) end
        end
    end
end

_emRun.Heartbeat:Connect(function()
    if not emoteState.enabled then return end
    if #_activeTracks == 0 and emoteState.animationId ~= "" then
        _emPlayEmote()
    end
    for _, t in ipairs(_activeTracks) do
        pcall(function() t:AdjustSpeed(emoteState.speed) end)
    end
end)

_emLP.CharacterAdded:Connect(function()
    task.wait(0.5)
...
-- ── Emote UI group — right column of Settings tab ────────────
local EmoteGroup = ConfigTab:Group("Emote")

-- Preset dropdown
local _emPresetNames = {"None"}
for _, n in ipairs(EMOTE_PRESETS_ORDER) do
    table.insert(_emPresetNames, n)
end
EmoteGroup:Dropdown({
    Name    = "Preset",
    Options = _emPresetNames,
    Default = "None",
    Callback = function(v)
        if v == "None" then
            emoteState.presetName   = nil
            emoteState.animationId  = ""
            emoteState.enabled      = emoteState.enabled and false or false
            _emStopAll()
        else
            emoteState.presetName  = v
            emoteState.animationId = EMOTE_PRESETS[v] or ""
            if emoteState.enabled then _emPlayEmote() end
        end
    end,
})
...
-- Speed slider  (1–10, maps to emote speed 0.5–5)
EmoteGroup:Slider({
    Name     = "Speed",
    Min      = 1,
    Max      = 10,
...
EmoteGroup:Button({
    ...
    Variant  = "Primary",
    Callback = function()
        if emoteState.animationId == "" then
            Library:Notify("No emote selected", "warning")
            return
        end
        emoteState.enabled = true
        _emPlayEmote()
        Library:Notify("Emote playing", "success")
    end,
})
